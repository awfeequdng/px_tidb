...
namespace mysql {
    ...
    class ErrMessage {
    public:
        ErrMessage(std::string raw, std::vector<int>&& redactArgs) : Raw(raw), RedactArgPos(std::move(redactArgs)) {}

        std::string Raw;
        std::vector<int> RedactArgPos;
    };

    std::shared_ptr<ErrMessage> Message(std::string message, std::vector<int>&& redactArgs);
