...
namespace errcode {
	...
	{ErrOutOfResources,                           mysql::Message("Out of memory; check if mysqld or some other process uses all available memory; if not, you may have to use 'ulimit' to allow mysqld to use more memory or you can add more swap space", {})},
	...
	{ErrFieldNotInGroupBy,                        mysql::Message("Expression #%d of %s is not in GROUP BY clause and contains nonaggregated column '%s' which is not functionally dependent on columns in GROUP BY clause; this is incompatible with sql_mode=only_full_group_by", {})},
	...
	{ErrBlobUsedAsKey,                            mysql::Message("BLOB column '%-.192s' can't be used in key specification with the used table type", {})},
	{ErrTooBigFieldlength,                        mysql::Message("Column length too big for column '%-.192s' (max = %d); use BLOB or TEXT instead", {})},
	{ErrWrongAutoKey,                             mysql::Message("Incorrect table definition; there can be only one auto column and it must be defined as a key", {})},
	...
	{ErrNoSuchIndex,                              mysql::Message("Table '%-.192s' has no index like the one used in CREATE INDEX; recreate the table", {})},
	{ErrWrongFieldTerminators,                    mysql::Message("Field separator argument is not what is expected; check the manual", {})},
	{ErrBlobsAndNoTerminated,                     mysql::Message("You can't use fixed rowlength with BLOBs; please use 'fields terminated by'", {})},
	{ErrTextFileNotReadable,                      mysql::Message("The file '%-.128s' must be in the database directory or be readable by all", {})},
	...
	{ErrWrongSubKey,                              mysql::Message("Incorrect prefix key; the used key part isn't a string, the used length is longer than the key part, or the storage engine doesn't support unique prefix keys", {})},
	{ErrCantRemoveAllFields,                      mysql::Message("You can't delete all columns with ALTER TABLE; use DROP TABLE instead", {})},
	...
	{ErrTooBigSelect,                             mysql::Message("The SELECT would examine more than MAXJOINSIZE rows; check your WHERE and use SET SQLBIGSELECTS=1 or SET MAXJOINSIZE=# if the SELECT is okay", {})},
	...
	{ErrUnsupportedExtension,                     mysql::Message("Table '%-.192s' uses an extension that doesn't exist in this MySQL version", {})},
	...
	{ErrTooBigRowsize,                            mysql::Message("Row size too large. The maximum row size for the used table type, not counting BLOBs, is %d. This includes storage overhead, check the manual. You have to change some columns to TEXT or BLOBs", {})},
	{ErrStackOverrun,                             mysql::Message("Thread stack overrun:  Used: %d of a %d stack.  Use 'mysqld --threadStack=#' to specify a bigger stack if needed", {})},
	{ErrWrongOuterJoin,                           mysql::Message("Cross dependency found in OUTER JOIN; examine your ON conditions", {})},
	{ErrNullColumnInIndex,                        mysql::Message("Table handler doesn't support NULL in given index. Please change column '%-.192s' to be NOT NULL or use another handler", {})},
	...
	{ErrHostIsBlocked,                            mysql::Message("Host '%-.255s' is blocked because of many connection errors; unblock with 'mysqladmin flush-hosts'", {})},
	{ErrHostNotPrivileged,                        mysql::Message("Host '%-.255s' is not allowed to connect to this MySQL server", {})},
	{ErrPasswordAnonymousUser,                    mysql::Message("You are using MySQL as an anonymous user and anonymous users are not allowed to change passwords", {})},
	{ErrPasswordNotAllowed,                       mysql::Message("You must have privileges to update tables in the mysql database to be able to change passwords for others", {})},
	...
	{ErrCantCreateThread,                         mysql::Message("Can't create a new thread (errno %d); if you are not out of available memory, you can consult the manual for a possible OS-dependent bug", {})},
	...
	{ErrMixOfGroupFuncAndFields,                  mysql::Message("Mixing of GROUP columns (MIN(),MAX(),COUNT(),...) with no GROUP columns is illegal if there is no GROUP BY clause", {})},
	...
	{ErrColumnaccessDenied,                       mysql::Message("%-.16s command denied to user '%-.48s'@'%-.255s' for column '%-.192s' in table '%-.192s'", {})},
	{ErrIllegalGrantForTable,                     mysql::Message("Illegal GRANT/REVOKE command; please consult the manual to see which privileges can be used", {})},
	...
	{ErrNonexistingTableGrant,                    mysql::Message("There is no such grant defined for user '%-.48s' on host '%-.255s' on table '%-.192s'", {})},
	{ErrNotAllowedCommand,                        mysql::Message("The used command is not allowed with this MySQL version", {})},
	{ErrSyntax,                                   mysql::Message("You have an error in your SQL syntax; check the manual that corresponds to your MySQL server version for the right syntax to use", {})},
	{ErrDelayedCantChangeLock,                    mysql::Message("Delayed insert thread couldn't get requested lock for table %-.192s", {})},
	...
	{ErrDelayedInsertTableLocked,                 mysql::Message("INSERT DELAYED can't be used with table '%-.192s' because it is locked with LOCK TABLES", {})},
	...
	{ErrWrongMrgTable,                            mysql::Message("Unable to open underlying table which is differently defined or of non-MyISAM type or doesn't exist", {})},
	{ErrDupUnique,                                mysql::Message("Can't write, because of unique constraint, to table '%-.192s'", {})},
	{ErrBlobKeyWithoutLength,                     mysql::Message("BLOB/TEXT column '%-.192s' used in key specification without a key length", {})},
	{ErrPrimaryCantHaveNull,                      mysql::Message("All parts of a PRIMARY KEY must be NOT NULL; if you need NULL in a key, use UNIQUE instead", {})},
	...
	{ErrUpdateWithoutKeyInSafeMode,               mysql::Message("You are using safe update mode and you tried to update a table without a WHERE that uses a KEY column", {})},
	...
	{ErrCantDoThisDuringAnTransaction,            mysql::Message("You are not allowed to execute this command in a transaction", {})},
	...
	{ErrNewAbortingConnection,                    mysql::Message("Aborted connection %d to db: '%-.192s' user: '%-.48s' host: '%-.255s' (%-.64s)", {})},
	...
	{ErrLockOrActiveTransaction,                  mysql::Message("Can't execute the given command because you have active locked tables or an active transaction", {})},
	...
	{ErrCrashedOnRepair,                          mysql::Message("Table '%-.192s' is marked as crashed and last (automatic?) repair failed", {})},
	{ErrWarningNotCompleteRollback,               mysql::Message("Some non-transactional changed tables couldn't be rolled back", {})},
	{ErrTransCacheFull,                           mysql::Message("Multi-statement transaction required more than 'maxBinlogCacheSize' bytes of storage; increase this mysqld variable and try again", {})},
	{ErrTooManyUserConnections,                   mysql::Message("User %-.64s already has more than 'maxUserConnections' active connections", {})},
	...
	{ErrReadOnlyTransaction,                      mysql::Message("Update locks cannot be acquired during a READ UNCOMMITTED transaction", {})},
	{ErrDropDBWithReadLock,                       mysql::Message("DROP DATABASE not allowed while thread is holding global read lock", {})},
	{ErrCreateDBWithReadLock,                     mysql::Message("CREATE DATABASE not allowed while thread is holding global read lock", {})},
	...
	{ErrUnionTablesInDifferentDir,                mysql::Message("Incorrect table definition; all MERGE tables must be in the same database", {})},
	...
	{ErrWrongNumberOfColumnsInSelect,             mysql::Message("The used SELECT statements have a different number of columns", {})},
	...
	{ErrSpecificAccessDenied,                     mysql::Message("Access denied; you need (at least one of) the %-.128s privilege(s) for this operation", {})},
	...
	{ErrNotSupportedAuthMode,                     mysql::Message("Client does not support authentication protocol requested by server; consider upgrading MySQL client", {})},
	...
	{ErrTooBigForUncompress,                      mysql::Message("Uncompressed data size too large; the maximum size is %d (probably, length of uncompressed data was corrupted)", {})},
	{ErrZlibZMem,                                 mysql::Message("ZLIB: Not enough memory", {})},
	{ErrZlibZBuf,                                 mysql::Message("ZLIB: Not enough room in the output buffer (probably, length of uncompressed data was corrupted)", {})},
	...
	{ErrWarnTooManyRecords,                       mysql::Message("Row %d was truncated; it contained more data than there were input columns", {})},
	{ErrWarnNullToNotnull,                        mysql::Message("Column set to default value; NULL supplied to NOT NULL column '%s' at row %d", {})},
	...
	{ErrCantAggregate2collations,                 mysql::Message("Illegal mix of collations (%s,%s) and (%s,%s) for operation '%s'", {})},
	...
	{ErrCantAggregate3collations,                 mysql::Message("Illegal mix of collations (%s,%s), (%s,%s), (%s,%s) for operation '%s'", {})},
	{ErrCantAggregateNcollations,                 mysql::Message("Illegal mix of collations for operation '%s'", {})},
	{ErrVariableIsNotStruct,                      mysql::Message("Variable '%-.64s' is not a variable component (can't be used as XXXX.variableName)", {})},
	{ErrUnknownCollation,                         mysql::Message("Unknown collation: '%-.64s'", {})},
	{ErrServerIsInSecureAuthMode,                 mysql::Message("Server is running in --secure-auth mode, but '%s'@'%s' has a password in the old format; please change the password to the new format", {})},
	{ErrWarnFieldResolved,                        mysql::Message("Field or reference '%-.192s%s%-.192s%s%-.192s' of SELECT #%d was resolved in SELECT #%d", {})},
	...
	{ErrWarnHostnameWontWork,                     mysql::Message("MySQL is started in --skip-name-resolve mode; you must restart it without this switch for this grant to work", {})},
	{ErrUnknownStorageEngine,                     mysql::Message("Unknown storage engine '%s'", {})},
	{ErrWarnDeprecatedSyntax,                     mysql::Message("'%s' is deprecated and will be removed in a future release. Please use %s instead", {})},
	{ErrNonUpdatableTable,                        mysql::Message("The target table %-.100s of the %s is not updatable", {})},
	{ErrFeatureDisabled,                          mysql::Message("The '%s' feature is disabled; you need MySQL built with '%s' to have it working", {})},
	{ErrOptionPreventsStatement,                  mysql::Message("The MySQL server is running with the %s option so it cannot execute this statement", {})},
	...
	{ErrTooMuchAutoTimestampCols,                 mysql::Message("Incorrect table definition; there can be only one TIMESTAMP column with CURRENTTIMESTAMP in DEFAULT or ON UPDATE clause", {})},
	...
	{ErrWarnAllowedPacketOverflowed,              mysql::Message("Result of %s() was larger than max_allowed_packet (%d) - truncated", {})},
	...
	{ErrUpdateLogDeprecatedIgnored,               mysql::Message("The update log is deprecated and replaced by the binary log; SET SQLLOGUPDATE has been ignored.", {})},
	{ErrUpdateLogDeprecatedTranslated,            mysql::Message("The update log is deprecated and replaced by the binary log; SET SQLLOGUPDATE has been translated to SET SQLLOGBIN.", {})},
	...
	{ErrSpVarcondAfterCurshndlr,                  mysql::Message("Variable or condition declaration after cursor or handler declaration", {})},
	...
	{ErrFparserEOFInUnknownParameter,             mysql::Message("Unexpected end of file while skipping unknown parameter '%-.192s'", {})},
	...
	{ErrViewWrongList,                            mysql::Message("In definition of view, derived table or common table expression, SELECT list and column names list have different column counts", {})},
	{ErrWarnViewMerge,                            mysql::Message("View merge algorithm can't be used here for now (assumed undefined algorithm)", {})},
	{ErrWarnViewWithoutKey,                       mysql::Message("View being updated does not have complete key of underlying table in it", {})},
	{ErrViewInvalid,                              mysql::Message("View '%-.192s.%-.192s' references invalid table(s) or column(s) or function(s) or definer/invoker of view lack rights to use them", {})},
	...
	{ErrTruncatedWrongValueForField,              mysql::Message("Incorrect %-.32s value: '%-.128s' for column '%.192s' at row %d", {0, 1})},
	...
	{ErrReservedSyntax,                           mysql::Message("The '%-.64s' syntax is reserved for purposes internal to the MySQL server", {})},
	...
	{ErrLoggingProhibitChangingOf,                mysql::Message("Binary logging and replication forbid changing the global server %s", {})},
	...
	{ErrViewMultiupdate,                          mysql::Message("Can not modify more than one base table through a join view '%-.192s.%-.192s'", {})},
	{ErrViewNoInsertFieldList,                    mysql::Message("Can not insert into join view '%-.192s.%-.192s' without fields list", {})},
	...
	{ErrXaerRmfail,                               mysql::Message("XAERRMFAIL: The command cannot be executed when global transaction is in the  %.64s state", {})},
	{ErrXaerOutside,                              mysql::Message("XAEROUTSIDE: Some work is done outside global transaction", {})},
	{ErrXaerRmerr,                                mysql::Message("XAERRMERR: Fatal error occurred in the transaction branch - check your data for consistency", {})},
	{ErrXaRbrollback,                             mysql::Message("XARBROLLBACK: Transaction branch was rolled back", {})},
	{ErrNonexistingProcGrant,                     mysql::Message("There is no such grant defined for user '%-.48s' on host '%-.255s' on routine '%-.192s'", {})},
	...
	{ErrSpNotVarArg,                              mysql::Message("OUT or INOUT argument %d for routine %s is not a variable or NEW pseudo-variable in BEFORE trigger", {})},
	{ErrSpNoRetset,                               mysql::Message("Not allowed to return a result set from a %s", {})},
	{ErrCantCreateGeometryObject,                 mysql::Message("Cannot get geometry object from data you send to the GEOMETRY field", {})},
	{ErrFailedRoutineBreakBinlog,                 mysql::Message("A routine failed and has neither NO SQL nor READS SQL DATA in its declaration and binary logging is enabled; if non-transactional tables were updated, the binary log will miss their changes", {})},
	{ErrBinlogUnsafeRoutine,                      mysql::Message("This function has none of DETERMINISTIC, NO SQL, or READS SQL DATA in its declaration and binary logging is enabled (you *might* want to use the less safe logBinTrustFunctionCreators variable)", {})},
	{ErrBinlogCreateRoutineNeedSuper,             mysql::Message("You do not have the SUPER privilege and binary logging is enabled (you *might* want to use the less safe logBinTrustFunctionCreators variable)", {})},
	{ErrExecStmtWithOpenCursor,                   mysql::Message("You can't execute a prepared statement which has an open cursor associated with it. Reset the statement to re-execute it.", {})},
	{ErrStmtHasNoOpenCursor,                      mysql::Message("The statement (%d) has no open cursor.", {})},
	{ErrCommitNotAllowedInSfOrTrg,                mysql::Message("Explicit or implicit commit is not allowed in stored function or trigger.", {})},
	{ErrNoDefaultForViewField,                    mysql::Message("Field of view '%-.192s.%-.192s' underlying table doesn't have a default value", {})},
	...
	{ErrMBiggerThanD,                             mysql::Message("For float(M,D), double(M,D) or decimal(M,D), M must be >= D (column '%-.192s').", {})},
	{ErrWrongLockOfSystemTable,                   mysql::Message("You can't combine write-locking of system tables with other tables or lock types", {})},
	{ErrConnectToForeignDataSource,               mysql::Message("Unable to connect to foreign data source: %.64s", {})},
	{ErrQueryOnForeignDataSource,                 mysql::Message("There was a problem processing the query on the foreign data source. Data source : %-.64s", {})},
	{ErrForeignDataSourceDoesntExist,             mysql::Message("The foreign data source you are trying to reference does not exist. Data source :  %-.64s", {})},
	{ErrForeignDataStringInvalidCantCreate,       mysql::Message("Can't create federated table. The data source connection string '%-.64s' is not in the correct format", {})},
	{ErrForeignDataStringInvalid,                 mysql::Message("The data source connection string '%-.64s' is not in the correct format", {})},
	...
	{ErrStackOverrunNeedMore,                     mysql::Message("Thread stack overrun:  %d bytes used of a %d byte stack, and %d bytes needed.  Use 'mysqld --threadStack=#' to specify a bigger stack.", {})},
	...
	{ErrCantUpdateUsedTableInSfOrTrg,             mysql::Message("Can't update table '%-.192s' in stored function/trigger because it is already used by statement which invoked this stored function/trigger.", {})},
	{ErrViewPreventUpdate,                        mysql::Message("The definition of table '%-.192s' prevents operation %.192s on table '%-.192s'.", {})},
	{ErrPsNoRecursion,                            mysql::Message("The prepared statement contains a stored routine call that refers to that same statement. It's not allowed to execute a prepared statement in such a recursive manner", {})},
	...
	{ErrViewFrmNoUser,                            mysql::Message("View '%-.192s'.'%-.192s' has no definer information (old table format). Current user is used as definer. Please recreate the view!", {})},
	{ErrViewOtherUser,                            mysql::Message("You need the SUPER privilege for creation view with '%-.192s'@'%-.255s' definer", {})},
	...
	{ErrRowIsReferenced2,                         mysql::Message("Cannot delete or update a parent row: a foreign key constraint fails (%.192s)", {})},
	{ErrNoReferencedRow2,                         mysql::Message("Cannot add or update a child row: a foreign key constraint fails (%.192s)", {})},
	{ErrSpBadVarShadow,                           mysql::Message("Variable '%-.64s' must be quoted with `...`, or renamed", {})},
	{ErrTrgNoDefiner,                             mysql::Message("No definer attribute for trigger '%-.192s'.'%-.192s'. The trigger will be activated under the authorization of the invoker, which may have insufficient privileges. Please recreate the trigger.", {})},
	{ErrOldFileFormat,                            mysql::Message("'%-.192s' has an old format, you should re-create the '%s' object(s)", {})},
	{ErrSpRecursionLimit,                         mysql::Message("Recursive limit %d (as set by the maxSpRecursionDepth variable) was exceeded for routine %.192s", {})},
	{ErrSpProcTableCorrupt,                       mysql::Message("Failed to load routine %-.192s. The table mysql.proc is missing, corrupt, or contains bad data (internal code %d)", {})},
	...
	{ErrMaxPreparedStmtCountReached,              mysql::Message("Can't create more than maxPreparedStmtCount statements (current value: %d)", {})},
	...
	{ErrAdminWrongMrgTable,                       mysql::Message("Table '%-.64s' is differently defined or of non-MyISAM type or doesn't exist", {})},
	...
	{ErrAmbiguousFieldTerm,                       mysql::Message("First character of the FIELDS TERMINATED string is ambiguous; please use non-optional and non-empty FIELDS ENCLOSED BY", {})},
	{ErrForeignServerExists,                      mysql::Message("The foreign server, %s, you are trying to create already exists.", {})},
	{ErrForeignServerDoesntExist,                 mysql::Message("The foreign server name you are trying to reference does not exist. Data source :  %-.64s", {})},
	{ErrIllegalHaCreateOption,                    mysql::Message("Table storage engine '%-.64s' does not support the create option '%.64s'", {})},
	{ErrPartitionRequiresValues,                  mysql::Message("Syntax : %-.64s PARTITIONING requires definition of VALUES %-.64s for each partition", {})},
	{ErrPartitionWrongValues,                     mysql::Message("Only %-.64s PARTITIONING can use VALUES %-.64s in partition definition", {1})},
	...
	{ErrPartitionWrongNoSubpart,                  mysql::Message("Wrong number of subpartitions defined, mismatch with previous setting", {})},
	{ErrWrongExprInPartitionFunc,                 mysql::Message("Constant, random or timezone-dependent expressions in (sub)partitioning function are not allowed", {})},
	...
	{ErrInconsistentPartitionInfo,                mysql::Message("The partition info in the frm file is not consistent with what can be written into the frm file", {})},
	...
	{ErrRangeNotIncreasing,                       mysql::Message("VALUES LESS THAN value must be strictly increasing for each partition", {})},
	...
	{ErrMixHandler,                               mysql::Message("The mix of handlers in the partitions is not allowed in this version of MySQL", {})},
	...
	{ErrSubpartition,                             mysql::Message("It is only possible to mix RANGE/LIST partitioning with HASH/KEY partitioning for subpartitioning", {})},
	...
	{ErrUniqueKeyNeedAllFieldsInPf,               mysql::Message("A %-.192s must include all columns in the table's partitioning function", {})},
	{ErrNoParts,                                  mysql::Message("Number of %-.64s = 0 is not an allowed value", {0})},
	{ErrPartitionMgmtOnNonpartitioned,            mysql::Message("Partition management on a not partitioned table is not possible", {})},
	{ErrForeignKeyOnPartitioned,                  mysql::Message("Foreign key clause is not yet supported in conjunction with partitioning", {})},
	...
	{ErrReorgHashOnlyOnSameNo,                    mysql::Message("REORGANIZE PARTITION can only be used to reorganize partitions not to change their numbers", {})},
	{ErrReorgNoParam,                             mysql::Message("REORGANIZE PARTITION without parameters can only be used on auto-partitioned tables using HASH PARTITIONs", {})},
	...
	{ErrConsecutiveReorgPartitions,               mysql::Message("When reorganizing a set of partitions they must be in consecutive order", {})},
	{ErrReorgOutsideRange,                        mysql::Message("Reorganize of range partitions cannot change total ranges except for last partition where it can extend the range", {})},
	{ErrPartitionFunctionFailure,                 mysql::Message("Partition function not supported in this version for this handler", {})},
	...
	{ErrWrongSizeNumber,                          mysql::Message("A size parameter was incorrectly specified, either number or on the form 10M", {})},
	{ErrSizeOverflow,                             mysql::Message("The size number was correct but we don't allow the digit part to be more than 2 billion", {})},
	...
	{ErrObsoleteColCountDoesntMatchCorrupted,     mysql::Message("Column count of mysql.%s is wrong. Expected %d, found %d. The table is probably corrupted", {})},
	...
	{ErrWarnDeprecatedSyntaxWithVer,              mysql::Message("The syntax '%s' is deprecated and will be removed in MySQL %s. Please use %s instead", {})},
	...
	{ErrForeignDuplicateKeyOldUnused,             mysql::Message("Upholding foreign key constraints for table '%.192s', entry '%-.192s', key %d would lead to a duplicate entry", {})},
	{ErrColCountDoesntMatchPleaseUpdate,          mysql::Message("Column count of mysql.%s is wrong. Expected %d, found %d. Created with MySQL %d, now running %d. Please use mysqlUpgrade to fix this error.", {})},
	{ErrTempTablePreventsSwitchOutOfRbr,          mysql::Message("Cannot switch out of the row-based binary log format when the session has open temporary tables", {})},
	{ErrStoredFunctionPreventsSwitchBinlogFormat, mysql::Message("Cannot change the binary logging format inside a stored function or trigger", {})},
	{ErrNdbCantSwitchBinlogFormat,                mysql::Message("The NDB cluster engine does not support changing the binlog format on the fly yet", {})},
	...
	{ErrCantChangeTxCharacteristics,              mysql::Message("Transaction characteristics can't be changed while a transaction is in progress", {})},
	{ErrDupEntryAutoincrementCase,                mysql::Message("ALTER TABLE causes autoIncrement resequencing, resulting in duplicate entry '%-.192s' for key '%-.192s'", {})},
	...
	{ErrEventRecursionForbidden,                  mysql::Message("Recursion of EVENT DDL statements is forbidden when body is present", {})},
	{ErrEventsDB,                                 mysql::Message("Cannot proceed because system tables used by Event Scheduler were found damaged at server start", {})},
	...
	{ErrCantRenameLogTable,                       mysql::Message("Cannot rename '%s'. When logging enabled, rename to/from log table must rename two tables: the log table to an archive table and another table back to '%s'", {})},
	{ErrWrongParamcountToNativeFct,               mysql::Message("Incorrect parameter count in the call to native function '%-.192s'", {})},
	{ErrWrongParametersToNativeFct,               mysql::Message("Incorrect parameters in the call to native function '%-.192s'", {})},
	{ErrWrongParametersToStoredFct,               mysql::Message("Incorrect parameters in the call to stored function '%-.192s'", {})},
	...
	{ErrEventCannotCreateInThePast,               mysql::Message("Event execution time is in the past and ON COMPLETION NOT PRESERVE is set. The event was dropped immediately after creation.", {})},
	{ErrEventCannotAlterInThePast,                mysql::Message("Event execution time is in the past and ON COMPLETION NOT PRESERVE is set. The event was not changed. Specify a time in the future.", {})},
	{ErrNoPartitionForGivenValueSilent,           mysql::Message("Table has no partition for some existing values", {})},
	{ErrBinlogUnsafeStatement,                    mysql::Message("Unsafe statement written to the binary log using statement format since BINLOGFORMAT = STATEMENT. %s", {})},
	...
	{ErrNoFormatDescriptionEventBeforeBinlogStatement,         mysql::Message("The BINLOG statement of type `%s` was not preceded by a format description BINLOG statement.", {})},
	...
	{ErrWarnEngineTransactionRollback,                         mysql::Message("Storage engine %s does not support rollback for this statement. Transaction rolled back and must be restarted", {})},
	...
	{ErrFuncInexistentNameCollision,                           mysql::Message("FUNCTION %s does not exist. Check the 'Function Name Parsing and Resolution' section in the Reference Manual", {})},
	...
	{ErrQueryCacheDisabled,                                    mysql::Message("Query cache is disabled; restart the server with queryCacheType=1 to enable it", {})},
	...
	{ErrRowSinglePartitionField,                               mysql::Message("Row expressions in VALUES IN only allowed for multi-field column partitioning", {})},
	{ErrFieldTypeNotAllowedAsPartitionField,                   mysql::Message("Field '%-.192s' is of a not allowed type for this type of partitioning", {})},
	{ErrPartitionFieldsTooLong,                                mysql::Message("The total length of the partitioning fields is too large", {})},
	{ErrBinlogRowEngineAndStmtEngine,                          mysql::Message("Cannot execute statement: impossible to write to binary log since both row-incapable engines and statement-incapable engines are involved.", {})},
	{ErrBinlogRowModeAndStmtEngine,                            mysql::Message("Cannot execute statement: impossible to write to binary log since BINLOGFORMAT = ROW and at least one table uses a storage engine limited to statement-based logging.", {})},
	{ErrBinlogUnsafeAndStmtEngine,                             mysql::Message("Cannot execute statement: impossible to write to binary log since statement is unsafe, storage engine is limited to statement-based logging, and BINLOGFORMAT = MIXED. %s", {})},
	{ErrBinlogRowInjectionAndStmtEngine,                       mysql::Message("Cannot execute statement: impossible to write to binary log since statement is in row format and at least one table uses a storage engine limited to statement-based logging.", {})},
	{ErrBinlogStmtModeAndRowEngine,                            mysql::Message("Cannot execute statement: impossible to write to binary log since BINLOGFORMAT = STATEMENT and at least one table uses a storage engine limited to row-based logging.%s", {})},
	{ErrBinlogRowInjectionAndStmtMode,                         mysql::Message("Cannot execute statement: impossible to write to binary log since statement is in row format and BINLOGFORMAT = STATEMENT.", {})},
	{ErrBinlogMultipleEnginesAndSelfLoggingEngine,             mysql::Message("Cannot execute statement: impossible to write to binary log since more than one engine is involved and at least one engine is self-logging.", {})},
	{ErrBinlogUnsafeLimit,                                     mysql::Message("The statement is unsafe because it uses a LIMIT clause. This is unsafe because the set of rows included cannot be predicted.", {})},
	{ErrBinlogUnsafeInsertDelayed,                             mysql::Message("The statement is unsafe because it uses INSERT DELAYED. This is unsafe because the times when rows are inserted cannot be predicted.", {})},
	{ErrBinlogUnsafeAutoincColumns,                            mysql::Message("Statement is unsafe because it invokes a trigger or a stored function that inserts into an AUTOINCREMENT column. Inserted values cannot be logged correctly.", {})},
	{ErrBinlogUnsafeNontransAfterTrans,                        mysql::Message("Statement is unsafe because it accesses a non-transactional table after accessing a transactional table within the same transaction.", {})},
	{ErrMessageAndStatement,                                   mysql::Message("%s Statement: %s", {})},
	{ErrInsideTransactionPreventsSwitchBinlogFormat,           mysql::Message("Cannot modify @@session.binlogFormat inside a transaction", {})},
	{ErrPathLength,                                            mysql::Message("The path specified for %.64s is too long.", {})},
	{ErrWarnDeprecatedSyntaxNoReplacement,                     mysql::Message("'%s' is deprecated and will be removed in a future release.", {})},
	...
	{ErrWarnISSkippedTable,                                    mysql::Message("Table '%s'.'%s' was skipped since its definition is being modified by concurrent DDL statement", {})},
	{ErrInsideTransactionPreventsSwitchBinlogDirect,           mysql::Message("Cannot modify @@session.binlogDirectNonTransactionalUpdates inside a transaction", {})},
	{ErrStoredFunctionPreventsSwitchBinlogDirect,              mysql::Message("Cannot change the binlog direct flag inside a stored function or trigger", {})},
	...
	{ErrBinlogUnsafeMultipleEnginesAndSelfLoggingEngine,       mysql::Message("Mixing self-logging and non-self-logging engines in a statement is unsafe.", {})},
	{ErrBinlogUnsafeMixedStatement,                            mysql::Message("Statement accesses nontransactional table as well as transactional or temporary table, and writes to any of them.", {})},
	{ErrInsideTransactionPreventsSwitchSQLLogBin,              mysql::Message("Cannot modify @@session.sqlLogBin inside a transaction", {})},
	{ErrStoredFunctionPreventsSwitchSQLLogBin,                 mysql::Message("Cannot change the sqlLogBin inside a stored function or trigger", {})},
	...
	{ErrSetPasswordAuthPlugin,                                 mysql::Message("SET PASSWORD has no significance for users authenticating via plugins", {})},
	{ErrGrantPluginUserExists,                                 mysql::Message("GRANT with IDENTIFIED WITH is illegal because the user %-.*s already exists", {})},
	{ErrTruncateIllegalFk,                                     mysql::Message("Cannot truncate a table referenced in a foreign key constraint (%.192s)", {})},
	...
	{ErrMultiUpdateKeyConflict,                                mysql::Message("Primary key/partition key update is not allowed since the table is updated both as '%-.192s' and '%-.192s'.", {})},
	{ErrTableNeedsRebuild,                                     mysql::Message("Table rebuild required. Please do \"ALTER TABLE `%-.32s` FORCE\" or dump/reload to fix it!", {})},
	...
	{ErrPluginNoUninstall,                                     mysql::Message("Plugin '%s' is marked as not dynamically uninstallable. You have to stop the server to uninstall it.", {})},
	{ErrPluginNoInstall,                                       mysql::Message("Plugin '%s' is marked as not dynamically installable. You have to stop the server to install it.", {})},
	{ErrBinlogUnsafeInsertTwoKeys,                             mysql::Message("INSERT... ON DUPLICATE KEY UPDATE  on a table with more than one UNIQUE KEY is unsafe", {})},
	...
	{ErrBinlogUnsafeAutoincNotFirst,                           mysql::Message("INSERT into autoincrement field which is not the first part in the composed primary key is unsafe.", {})},
	{ErrCannotLoadFromTableV2,                                 mysql::Message("Cannot load from %s.%s. The table is probably corrupted", {})},
	{ErrOnlyFdAndRbrEventsAllowedInBinlogStatement,            mysql::Message("Only FormatDescriptionLogEvent and row events are allowed in BINLOG statements (but %s was provided)", {})},
	{ErrPartitionExchangeDifferentOption,                      mysql::Message("Non matching attribute '%-.64s' between partition and table", {})},
	...
	{ErrPartitionInsteadOfSubpartition,                        mysql::Message("Subpartitioned table, use subpartition instead of partition", {})},
	...
	{ErrBinlogCacheSizeGreaterThanMax,                         mysql::Message("Option binlogCacheSize (%d) is greater than maxBinlogCacheSize (%d); setting binlogCacheSize equal to maxBinlogCacheSize.", {})},
	{ErrWarnIndexNotApplicable,                                mysql::Message("Cannot use %-.64s access on index '%-.64s' due to type or collation conversion on field '%-.64s'", {})},
	{ErrPartitionExchangeForeignKey,                           mysql::Message("Table to exchange with partition has foreign key references: '%-.64s'", {})},
	...
	{ErrNetworkReadEventChecksumFailure,                       mysql::Message("Replication event checksum verification failed while reading from network.", {})},
	{ErrBinlogReadEventChecksumFailure,                        mysql::Message("Replication event checksum verification failed while reading from a log file.", {})},
	{ErrBinlogStmtCacheSizeGreaterThanMax,                     mysql::Message("Option binlogStmtCacheSize (%d) is greater than maxBinlogStmtCacheSize (%d); setting binlogStmtCacheSize equal to maxBinlogStmtCacheSize.", {})},
	{ErrCantUpdateTableInCreateTableSelect,                    mysql::Message("Can't update table '%-.192s' while '%-.192s' is being created.", {})},
	...
	{ErrChangeRplInfoRepositoryFailure,                        mysql::Message("Failure while changing the type of replication repository: %s.", {})},
	{ErrWarningNotCompleteRollbackWithCreatedTempTable,        mysql::Message("The creation of some temporary tables could not be rolled back.", {})},
	{ErrWarningNotCompleteRollbackWithDroppedTempTable,        mysql::Message("Some temporary tables were dropped, but these operations could not be rolled back.", {})},
	{ErrMtsUpdatedDBsGreaterMax,                               mysql::Message("The number of modified databases exceeds the maximum %d; the database names will not be included in the replication event metadata.", {})},
	{ErrMtsCantParallel,                                       mysql::Message("Cannot execute the current event group in the parallel mode. Encountered event %s, relay-log name %s, position %s which prevents execution of this event group in parallel mode. Reason: %s.", {})},
	{ErrMtsInconsistentData,                                   mysql::Message("%s", {})},
	{ErrFulltextNotSupportedWithPartitioning,                  mysql::Message("FULLTEXT index is not supported for partitioned tables.", {})},
	{ErrDaInvalidConditionNumber,                              mysql::Message("Invalid condition number", {})},
	{ErrInsecurePlainText,                                     mysql::Message("Sending passwords in plain text without SSL/TLS is extremely insecure.", {})},
	{ErrForeignDuplicateKeyWithChildInfo,                      mysql::Message("Foreign key constraint for table '%.192s', record '%-.192s' would lead to a duplicate entry in table '%.192s', key '%.192s'", {})},
	{ErrForeignDuplicateKeyWithoutChildInfo,                   mysql::Message("Foreign key constraint for table '%.192s', record '%-.192s' would lead to a duplicate entry in a child table", {})},
	{ErrTableHasNoFt,                                          mysql::Message("The table does not have FULLTEXT index to support this query", {})},
	{ErrVariableNotSettableInSfOrTrigger,                      mysql::Message("The system variable %.200s cannot be set in stored functions or triggers.", {})},
	{ErrVariableNotSettableInTransaction,                      mysql::Message("The system variable %.200s cannot be set when there is an ongoing transaction.", {})},
	{ErrGtidNextIsNotInGtidNextList,                           mysql::Message("The system variable @@SESSION.GTIDNEXT has the value %.200s, which is not listed in @@SESSION.GTIDNEXTLIST.", {})},
	{ErrCantChangeGtidNextInTransactionWhenGtidNextListIsNull, mysql::Message("When @@SESSION.GTIDNEXTLIST == NULL, the system variable @@SESSION.GTIDNEXT cannot change inside a transaction.", {})},
	{ErrSetStatementCannotInvokeFunction,                      mysql::Message("The statement 'SET %.200s' cannot invoke a stored function.", {})},
	{ErrGtidNextCantBeAutomaticIfGtidNextListIsNonNull,        mysql::Message("The system variable @@SESSION.GTIDNEXT cannot be 'AUTOMATIC' when @@SESSION.GTIDNEXTLIST is non-NULL.", {})},
	{ErrSkippingLoggedTransaction,                             mysql::Message("Skipping transaction %.200s because it has already been executed and logged.", {})},
	...
	{ErrGnoExhausted,                                          mysql::Message("Impossible to generate Global Transaction Identifier: the integer component reached the maximal value. Restart the server with a new serverUuid.", {})},
	{ErrCantDoImplicitCommitInTrxWhenGtidNextIsSet,            mysql::Message("Cannot execute statements with implicit commit inside a transaction when @@SESSION.GTIDNEXT != AUTOMATIC or @@SESSION.GTIDNEXTLIST != NULL.", {})},
	{ErrGtidMode2Or3RequiresEnforceGtidConsistencyOn,          mysql::Message("@@GLOBAL.GTIDMODE = ON or UPGRADESTEP2 requires @@GLOBAL.ENFORCEGTIDCONSISTENCY = 1.", {})},
	{ErrCantSetGtidNextToGtidWhenGtidModeIsOff,                mysql::Message("@@SESSION.GTIDNEXT cannot be set to UUID:NUMBER when @@GLOBAL.GTIDMODE = OFF.", {})},
	{ErrCantSetGtidNextToAnonymousWhenGtidModeIsOn,            mysql::Message("@@SESSION.GTIDNEXT cannot be set to ANONYMOUS when @@GLOBAL.GTIDMODE = ON.", {})},
	{ErrCantSetGtidNextListToNonNullWhenGtidModeIsOff,         mysql::Message("@@SESSION.GTIDNEXTLIST cannot be set to a non-NULL value when @@GLOBAL.GTIDMODE = OFF.", {})},
	{ErrFoundGtidEventWhenGtidModeIsOff,                       mysql::Message("Found a GtidLogEvent or PreviousGtidsLogEvent when @@GLOBAL.GTIDMODE = OFF.", {})},
	{ErrGtidUnsafeNonTransactionalTable,                       mysql::Message("When @@GLOBAL.ENFORCEGTIDCONSISTENCY = 1, updates to non-transactional tables can only be done in either autocommitted statements or single-statement transactions, and never in the same statement as updates to transactional tables.", {})},
	{ErrGtidUnsafeCreateSelect,                                mysql::Message("CREATE TABLE ... SELECT is forbidden when @@GLOBAL.ENFORCEGTIDCONSISTENCY = 1.", {})},
	{ErrGtidUnsafeCreateDropTemporaryTableInTransaction,       mysql::Message("When @@GLOBAL.ENFORCEGTIDCONSISTENCY = 1, the statements CREATE TEMPORARY TABLE and DROP TEMPORARY TABLE can be executed in a non-transactional context only, and require that AUTOCOMMIT = 1.", {})},
	{ErrGtidModeCanOnlyChangeOneStepAtATime,                   mysql::Message("The value of @@GLOBAL.GTIDMODE can only change one step at a time: OFF <-> UPGRADESTEP1 <-> UPGRADESTEP2 <-> ON. Also note that this value must be stepped up or down simultaneously on all servers; see the Manual for instructions.", {})},
	{ErrCantSetGtidNextWhenOwningGtid,                         mysql::Message("@@SESSION.GTIDNEXT cannot be changed by a client that owns a GTID. The client owns %s. Ownership is released on COMMIT or ROLLBACK.", {})},
	...
	{ErrTooLongTablePartitionComment,                          mysql::Message("Comment for table partition '%-.64s' is too long (max = %d)", {})},
	...
	{ErrInnodbOnlineLogTooBig,                                 mysql::Message("Creating index '%-.192s' required more than 'innodbOnlineAlterLogMaxSize' bytes of modification log. Please try again.", {})},
	{ErrUnknownAlterAlgorithm,                                 mysql::Message("Unknown ALGORITHM '%s'", {})},
	{ErrUnknownAlterLock,                                      mysql::Message("Unknown LOCK type '%s'", {})},
	{ErrMtsResetWorkers,                                       mysql::Message("Cannot clean up worker info tables. Additional error messages can be found in the MySQL error log.", {})},
	{ErrColCountDoesntMatchCorruptedV2,                        mysql::Message("Column count of %s.%s is wrong. Expected %d, found %d. The table is probably corrupted", {})},
	{ErrDiscardFkChecksRunning,                                mysql::Message("There is a foreign key check running on table '%-.192s'. Cannot discard the table.", {})},
	...
	{ErrTablespaceExists,                                      mysql::Message("Tablespace for table '%-.192s' exists. Please DISCARD the tablespace before IMPORT.", {})},
	...
	{ErrFkNoIndexChild,                                        mysql::Message("Failed to add the foreign key constaint. Missing index for constraint '%s' in the foreign table '%s'", {})},
	{ErrFkNoIndexParent,                                       mysql::Message("Failed to add the foreign key constaint. Missing index for constraint '%s' in the referenced table '%s'", {})},
	...
	{ErrFkIncorrectOption,                                     mysql::Message("Failed to add the foreign key constraint on table '%s'. Incorrect options in FOREIGN KEY constraint '%s'", {})},
	{ErrFkDupName,                                             mysql::Message("Duplicate foreign key constraint name '%s'", {})},
	{ErrPasswordFormat,                                        mysql::Message("The password hash doesn't have the expected format. Check if the correct password algorithm is being used with the PASSWORD() function.", {})},
	{ErrFkColumnCannotDrop,                                    mysql::Message("Cannot drop column '%-.192s': needed in a foreign key constraint '%-.192s'", {})},
	{ErrFkColumnCannotDropChild,                               mysql::Message("Cannot drop column '%-.192s': needed in a foreign key constraint '%-.192s' of table '%-.192s'", {})},
	{ErrFkColumnNotNull,                                       mysql::Message("Column '%-.192s' cannot be NOT NULL: needed in a foreign key constraint '%-.192s' SET NULL", {})},
	{ErrDupIndex,                                              mysql::Message("Duplicate index '%-.64s' defined on the table '%-.64s.%-.64s'. This is deprecated and will be disallowed in a future release.", {})},
	{ErrFkColumnCannotChange,                                  mysql::Message("Cannot change column '%-.192s': used in a foreign key constraint '%-.192s'", {})},
	{ErrFkColumnCannotChangeChild,                             mysql::Message("Cannot change column '%-.192s': used in a foreign key constraint '%-.192s' of table '%-.192s'", {})},
	{ErrFkCannotDeleteParent,                                  mysql::Message("Cannot delete rows from table which is parent in a foreign key constraint '%-.192s' of table '%-.192s'", {})},
	...
	{ErrCantSetGtidPurgedWhenGtidModeIsOff,                    mysql::Message("@@GLOBAL.GTIDPURGED can only be set when @@GLOBAL.GTIDMODE = ON.", {})},
	{ErrCantSetGtidPurgedWhenGtidExecutedIsNotEmpty,           mysql::Message("@@GLOBAL.GTIDPURGED can only be set when @@GLOBAL.GTIDEXECUTED is empty.", {})},
	{ErrCantSetGtidPurgedWhenOwnedGtidsIsNotEmpty,             mysql::Message("@@GLOBAL.GTIDPURGED can only be set when there are no ongoing transactions (not even in other clients).", {})},
	...
	{ErrBinlogStmtModeAndNoReplTables,                         mysql::Message("Cannot execute statement: impossible to write to binary log since BINLOGFORMAT = STATEMENT, and both replicated and non replicated tables are written to.", {})},
	...
	{ErrAlterOperationNotSupportedReasonPartition,             mysql::Message("Partition specific operations do not yet support LOCK/ALGORITHM", {})},
	{ErrAlterOperationNotSupportedReasonFkRename,              mysql::Message("Columns participating in a foreign key are renamed", {})},
	...
	{ErrAlterOperationNotSupportedReasonIgnore,                mysql::Message("Creating unique indexes with IGNORE requires COPY algorithm to remove duplicate rows", {})},
	{ErrAlterOperationNotSupportedReasonNopk,                  mysql::Message("Dropping a primary key is not allowed without also adding a new primary key", {})},
	{ErrAlterOperationNotSupportedReasonAutoinc,               mysql::Message("Adding an auto-increment column requires a lock", {})},
	{ErrAlterOperationNotSupportedReasonHiddenFts,             mysql::Message("Cannot replace hidden FTSDOCID with a user-visible one", {})},
	...
	{ErrIdentCausesTooLongPath,                                mysql::Message("Long database name and identifier for object resulted in path length exceeding %d characters. Path: '%s'.", {})},
	{ErrAlterOperationNotSupportedReasonNotNull,               mysql::Message("cannot silently convert NULL values, as required in this SQLMODE", {})},
	{ErrMustChangePasswordLogin,                               mysql::Message("Your password has expired. To log in you must change it using a client that supports expired passwords.", {})},
	{ErrRowInWrongPartition,                                   mysql::Message("Found a row in wrong partition %s", {0})},
	{ErrGeneratedColumnFunctionIsNotAllowed,                   mysql::Message("Expression of generated column '%s' contains a disallowed function.", {})},
	{ErrGeneratedColumnRowValueIsNotAllowed,                   mysql::Message("Expression of generated column '%s' cannot refer to a row value", {})},
	{ErrUnsupportedAlterInplaceOnVirtualColumn,                mysql::Message("INPLACE ADD or DROP of virtual columns cannot be combined with other ALTER TABLE actions.", {})},
	{ErrWrongFKOptionForGeneratedColumn,                       mysql::Message("Cannot define foreign key with %s clause on a generated column.", {})},
	{ErrBadGeneratedColumn,                                    mysql::Message("The value specified for generated column '%s' in table '%s' is not allowed.", {})},
	{ErrUnsupportedOnGeneratedColumn,                          mysql::Message("'%s' is not supported for generated columns.", {})},
	{ErrGeneratedColumnNonPrior,                               mysql::Message("Generated column can refer only to generated columns defined prior to it.", {})},
	...
	{ErrAggregateOrderNonAggQuery,                             mysql::Message("Expression #%d of ORDER BY contains aggregate function and applies to the result of a non-aggregated query", {})},
	{ErrIncorrectType,                                         mysql::Message("Incorrect type for argument %s in function %s.", {})},
	{ErrFieldInOrderNotSelect,                                 mysql::Message("Expression #%d of ORDER BY clause is not in SELECT list, references column '%s' which is not in SELECT list; this is incompatible with %s", {})},
	{ErrAggregateInOrderNotSelect,                             mysql::Message("Expression #%d of ORDER BY clause is not in SELECT list, contains aggregate function; this is incompatible with %s", {})},
	...
	{ErrInvalidTypeForJSON,                                    mysql::Message("Invalid data type for JSON data in argument %d to function %s; a JSON string or JSON type is required.", {})},
	{ErrInvalidJSONPathWildcard,                               mysql::Message("In this situation, path expressions may not contain the * and ** tokens.", {})},
	...
	{ErrSecureTransportRequired,                               mysql::Message("Connections using insecure transport are prohibited while --require_secure_transport=ON.", {})},
	...
	{ErrWindowNoInherentFrame,                                 mysql::Message("Window '%s' has a frame definition, so cannot be referenced by another window.", {})},
	{ErrWindowNoRedefineOrderBy,                               mysql::Message("Window '%s' cannot inherit '%s' since both contain an ORDER BY clause.", {})},
	...
	{ErrWindowFrameIllegal,                                    mysql::Message("Window '%s': frame start or end is negative, NULL or of non-integral type", {})},
	{ErrWindowRangeFrameOrderType,                             mysql::Message("Window '%s' with RANGE N PRECEDING/FOLLOWING frame requires exactly one ORDER BY expression, of numeric or temporal type", {})},
	{ErrWindowRangeFrameTemporalType,                          mysql::Message("Window '%s' with RANGE frame has ORDER BY expression of datetime type. Only INTERVAL bound value allowed.", {})},
	{ErrWindowRangeFrameNumericType,                           mysql::Message("Window '%s' with RANGE frame has ORDER BY expression of numeric type, INTERVAL bound value not allowed.", {})},
	...
	{ErrWindowIllegalOrderBy,                                  mysql::Message("Window '%s': ORDER BY or PARTITION BY uses legacy position indication which is not supported, use expression.", {})},
	{ErrWindowInvalidWindowFuncUse,                            mysql::Message("You cannot use the window function '%s' in this context.'", {})},
	{ErrWindowInvalidWindowFuncAliasUse,                       mysql::Message("You cannot use the alias '%s' of an expression containing a window function in this context.'", {})},
	{ErrWindowNestedWindowFuncUseInWindowSpec,                 mysql::Message("You cannot nest a window function in the specification of window '%s'.", {})},
	{ErrWindowRowsIntervalUse,                                 mysql::Message("Window '%s': INTERVAL can only be used with RANGE frames.", {})},
	{ErrWindowNoGroupOrderUnused,                              mysql::Message("ASC or DESC with GROUP BY isn't allowed with window functions; put ASC or DESC in ORDER BY", {})},
	{ErrWindowExplainJSON,                                     mysql::Message("To get information about window functions use EXPLAIN FORMAT=JSON", {})},
	{ErrWindowFunctionIgnoresFrame,                            mysql::Message("Window function '%s' ignores the frame clause of window '%s' and aggregates over the whole partition", {})},
	...
	{ErrLockAcquireFailAndNoWaitSet,                           mysql::Message("Statement aborted because lock(s) could not be acquired immediately and NOWAIT is set.", {})},
	...
	{ErrFunctionalIndexOnJSONOrGeometryFunction,               mysql::Message("Cannot create an expression index on a function that returns a JSON or GEOMETRY value", {})},
	{ErrFunctionalIndexRefAutoIncrement,                       mysql::Message("Expression index '%s' cannot refer to an auto-increment column", {})},
	{ErrCannotDropColumnFunctionalIndex,                       mysql::Message("Cannot drop column '%s' because it is used by an expression index. In order to drop the column, you must remove the expression index", {})},
	{ErrFunctionalIndexPrimaryKey,                             mysql::Message("The primary key cannot be an expression index", {})},
	{ErrFunctionalIndexOnBlob,                                 mysql::Message("Cannot create an expression index on an expression that returns a BLOB or TEXT. Please consider using CAST", {})},
	{ErrFunctionalIndexFunctionIsNotAllowed,                   mysql::Message("Expression of expression index '%s' contains a disallowed function", {})},
	...
	{ErrFunctionalIndexOnField,                                mysql::Message("Expression index on a column is not supported. Consider using a regular index instead", {})},
	{ErrFKIncompatibleColumns,                                 mysql::Message("Referencing column '%s' in foreign key constraint '%s' are incompatible", {})},
	{ErrFunctionalIndexRowValueIsNotAllowed,                   mysql::Message("Expression of expression index '%s' cannot refer to a row value", {})},
	{ErrDependentByFunctionalIndex,                            mysql::Message("Column '%s' has an expression index dependency and cannot be dropped or renamed", {})},
	{ErrInvalidJSONValueForFuncIndex,                          mysql::Message("Invalid JSON value for CAST for expression index '%s'", {})},
	{ErrJSONValueOutOfRangeForFuncIndex,                       mysql::Message("Out of range JSON value for CAST for expression index '%s'", {})},
	{ErrFunctionalIndexDataIsTooLong,                          mysql::Message("Data too long for expression index '%s'", {})},
	{ErrFunctionalIndexNotApplicable,                          mysql::Message("Cannot use expression index '%s' due to type or collation conversion", {})},
	...
	{ErrCTERecursiveRequiresNonRecursiveFirst,                 mysql::Message("Recursive Common Table Expression '%s' should have one or more non-recursive query blocks followed by one or more recursive ones", {})},
	{ErrCTERecursiveForbidsAggregation,                        mysql::Message("Recursive Common Table Expression '%s' can contain neither aggregation nor window functions in recursive query block", {})},
	{ErrCTERecursiveForbiddenJoinOrder,                        mysql::Message("In recursive query block of Recursive Common Table Expression '%s', the recursive table must neither be in the right argument of a LEFT JOIN, nor be forced to be non-first with join order hints", {})},
	{ErrInvalidRequiresSingleReference,                        mysql::Message("In recursive query block of Recursive Common Table Expression '%s', the recursive table must be referenced only once, and not in any subquery", {})},
	{ErrCTEMaxRecursionDepth,                                  mysql::Message("Recursive query aborted after %d iterations. Try increasing @@cte_max_recursion_depth to a larger value", {})},
	// MariaDB errors.
	{ErrOnlyOneDefaultPartionAllowed,         mysql::Message("Only one DEFAULT partition allowed", {})},
	{ErrWrongPartitionTypeExpectedSystemTime, mysql::Message("Wrong partitioning type, expected type: `SYSTEM_TIME`", {})},
	{ErrSystemVersioningWrongPartitions,      mysql::Message("Wrong Partitions: must have at least one HISTORY and exactly one last CURRENT", {})},
	...
	{ErrWrongInsertIntoSequence,              mysql::Message("Wrong INSERT into a SEQUENCE. One can only do single table INSERT into a sequence object (like with mysqldump).  If you want to change the SEQUENCE, use ALTER SEQUENCE instead.", {})},
	...
	{ErrDropTableOnTemporaryTable,     mysql::Message("`drop global temporary table` can only drop global temporary table", {})},
	...
	{ErrInvalidPluginID,               mysql::Message("Wrong plugin id: %s, valid plugin id is [name]-[version], both name and version should not contain '-'", {})},
	...
	{ErrInfoSchemaExpired,             mysql::Message("Information schema is out of date: schema failed to update in 1 lease, please make sure TiDB can connect to TiKV", {})},
	{ErrInfoSchemaChanged,             mysql::Message("Information schema is changed during the execution of the statement(for example, table definition may be updated by other DDL ran in parallel). If you see this error often, try increasing `tidb_max_delta_schema_count`", {})},
	{ErrBadNumber,                     mysql::Message("Bad Number", {})},
	{ErrCastAsSignedOverflow,          mysql::Message("Cast to signed converted positive out-of-range integer to it's negative complement", {})},
	{ErrCastNegIntAsUnsigned,          mysql::Message("Cast to unsigned converted negative integer to it's positive complement", {})},
	...
	{ErrUnsupportedIsolationLevel,     mysql::Message("The isolation level '%s' is not supported. Set tidb_skip_isolation_level_check=1 to skip this error", {})},
	...
	{ErrInvalidIncrementAndOffset,     mysql::Message("Invalid auto_increment settings: auto_increment_increment: %d, auto_increment_offset: %d, both of them must be in range [1..65535]", {})},
	{ErrDataInConsistentExtraIndex,    mysql::Message("handle %#v, index:%#v != record:%#v", {0, 1, 2})},
	{ErrDataInConsistentMisMatchIndex, mysql::Message("col %s, handle %#v, index:%#v != record:%#v, compare err:%#v", {1, 2, 3, 4})},

	{ErrWarnOptimizerHintInvalidInteger,  mysql::Message("integer value is out of range in '%s'", {})},
	{ErrWarnOptimizerHintUnsupportedHint, mysql::Message("Optimizer hint %s is not supported by TiDB and is ignored", {})},
	{ErrWarnOptimizerHintInvalidToken,    mysql::Message("Cannot use %s '%s' (tok = %d) in an optimizer hint", {})},
	{ErrWarnMemoryQuotaOverflow,          mysql::Message("Max value of MEMORY_QUOTA is %d bytes, ignore this invalid limit", {})},
	...
	{ErrAddColumnWithSequenceAsDefault,      mysql::Message("Unsupported using sequence as default value in add column '%s'", {})},
	...
	{ErrPrivilegeCheckFail,                  mysql::Message("privilege check for '%s' fail", {})}, // this error message should begin lowercased to be compatible with the test
	{ErrInvalidWildCard,                     mysql::Message("Wildcard fields without any table name appears in wrong place", {})},
	{ErrMixOfGroupFuncAndFieldsIncompatible, mysql::Message("In aggregated query without GROUP BY, expression #%d of SELECT list contains nonaggregated column '%s'; this is incompatible with sql_mode=only_full_group_by", {})},
	{ErrUnsupportedSecondArgumentType,       mysql::Message("JSON_OBJECTAGG: unsupported second argument type %v", {})},
	{ErrLockExpire,                          mysql::Message("TTL manager has timed out, pessimistic locks may expire, please commit or rollback this transaction", {})},
	{ErrTableOptionUnionUnsupported,         mysql::Message("CREATE/ALTER table with union option is not supported", {})},
	{ErrTableOptionInsertMethodUnsupported,  mysql::Message("CREATE/ALTER table with insert method option is not supported", {})},
	...
	{ErrJSONObjectKeyTooLong,  mysql::Message("TiDB does not yet support JSON objects with the key length >= 65536", {})},
	{ErrPartitionStatsMissing, mysql::Message("Build table: %s global-level stats failed due to missing partition-level stats", {})},
	{ErrNotSupportedWithSem,   mysql::Message("Feature '%s' is not supported when security enhanced mode is enabled", {})},
	...
	{ErrMultiStatementDisabled,          mysql::Message("client has multi-statement capability disabled. Run SET GLOBAL tidb_multi_statement_mode='ON' after you understand the security risk", {})},
	...
	{ErrPlacementPolicyWithDirectOption, mysql::Message("Placement policy '%s' can't co-exist with direct placement options", {})},
	...
	{ErrGCTooEarly,                mysql::Message("GC life time is shorter than transaction duration, transaction starts at %v, GC safe point is %v", {})},
	{ErrWriteConflict,             mysql::Message("Write conflict, txnStartTS=%d, conflictStartTS=%d, conflictCommitTS=%d, key=%s", {3})},
