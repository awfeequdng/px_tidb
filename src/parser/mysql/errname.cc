...
unordered_map<uint16_t, shared_ptr<ErrMessage>> MySQLErrName = {
        ...
        {ErrOutOfResources,                           Message("Out of memory; check if mysqld or some other process uses all available memory; if not, you may have to use 'ulimit' to allow mysqld to use more memory or you can add more swap space", {})},
        ...
        {ErrFieldNotInGroupBy,                        Message("Expression #%d of %s is not in GROUP BY clause and contains nonaggregated column '%s' which is not functionally dependent on columns in GROUP BY clause; this is incompatible with sql_mode=only_full_group_by", {})},
        ...
        {ErrBlobUsedAsKey,                            Message("BLOB column '%-.192s' can't be used in key specification with the used table type", {})},
        {ErrTooBigFieldlength,                        Message("Column length too big for column '%-.192s' (max = %d); use BLOB or TEXT instead", {})},
        {ErrWrongAutoKey,                             Message("Incorrect table definition; there can be only one auto column and it must be defined as a key", {})},
        ...
        {ErrBlobsAndNoTerminated,                     Message("You can't use fixed rowlength with BLOBs; please use 'fields terminated by'", {})},
        ...
        {ErrWrongSubKey,                              Message("Incorrect prefix key; the used key part isn't a string, the used length is longer than the key part, or the storage engine doesn't support unique prefix keys", {})},
        ...
        {ErrTooBigSelect,                             Message("The SELECT would examine more than MAXJOINSIZE rows; check your WHERE and use SET SQLBIGSELECTS=1 or SET MAXJOINSIZE=# if the SELECT is okay", {})},
        ...
        {ErrUnsupportedExtension,                     Message("Table '%-.192s' uses an extension that doesn't exist in this MySQL version", {})},
        ...
        {ErrTooBigRowsize,                            Message("Row size too large. The maximum row size for the used table type, not counting BLOBs, is %d. This includes storage overhead, check the manual. You have to change some columns to TEXT or BLOBs", {})},
        {ErrStackOverrun,                             Message("Thread stack overrun:  Used: %d of a %d stack.  Use 'mysqld --threadStack=#' to specify a bigger stack if needed", {})},
        {ErrWrongOuterJoin,                           Message("Cross dependency found in OUTER JOIN; examine your ON conditions", {})},
        {ErrNullColumnInIndex,                        Message("Table handler doesn't support NULL in given index. Please change column '%-.192s' to be NOT NULL or use another handler", {})},
        ...
        {ErrHostIsBlocked,                            Message("Host '%-.64s' is blocked because of many connection errors; unblock with 'mysqladmin flush-hosts'", {})},
        {ErrHostNotPrivileged,                        Message("Host '%-.64s' is not allowed to connect to this MySQL server", {})},
        {ErrPasswordAnonymousUser,                    Message("You are using MySQL as an anonymous user and anonymous users are not allowed to change passwords", {})},
        {ErrPasswordNotAllowed,                       Message("You must have privileges to update tables in the mysql database to be able to change passwords for others", {})},
        ...
        {ErrCantCreateThread,                         Message("Can't create a new thread (errno %d); if you are not out of available memory, you can consult the manual for a possible OS-dependent bug", {})},
        ...
        {ErrMixOfGroupFuncAndFields,                  Message("Mixing of GROUP columns (MIN(),MAX(),COUNT(),...) with no GROUP columns is illegal if there is no GROUP BY clause", {})},
        ...
        {ErrColumnaccessDenied,                       Message("%-.16s command denied to user '%-.48s'@'%-.64s' for column '%-.192s' in table '%-.192s'", {})},
        {ErrIllegalGrantForTable,                     Message("Illegal GRANT/REVOKE command; please consult the manual to see which privileges can be used", {})},
        ...
        {ErrNonexistingTableGrant,                    Message("There is no such grant defined for user '%-.48s' on host '%-.64s' on table '%-.192s'", {})},
        {ErrNotAllowedCommand,                        Message("The used command is not allowed with this MySQL version", {})},
        {ErrSyntax,                                   Message("You have an error in your SQL syntax; check the manual that corresponds to your MySQL server version for the right syntax to use", {})},
        ...
        {ErrDelayedInsertTableLocked,                 Message("INSERT DELAYED can't be used with table '%-.192s' because it is locked with LOCK TABLES", {})},
        ...
        {ErrWrongMrgTable,                            Message("Unable to open underlying table which is differently defined or of non-MyISAM type or doesn't exist", {})},
        ...
        {ErrPrimaryCantHaveNull,                      Message("All parts of a PRIMARY KEY must be NOT NULL; if you need NULL in a key, use UNIQUE instead", {})},
        ...
        {ErrUpdateWithoutKeyInSafeMode,               Message("You are using safe update mode and you tried to update a table without a WHERE that uses a KEY column", {})},
        ...
        {ErrNewAbortingConnection,                    Message("Aborted connection %d to db: '%-.192s' user: '%-.48s' host: '%-.64s' (%-.64s)", {})},
        ...
        {ErrLockOrActiveTransaction,                  Message("Can't execute the given command because you have active locked tables or an active transaction", {})},
        ...
        {ErrTransCacheFull,                           Message("Multi-statement transaction required more than 'maxBinlogCacheSize' bytes of storage; increase this mysqld variable and try again", {})},
        ...
        {ErrMasterInfo,                               Message("Could not initialize master info structure; more error messages can be found in the MySQL error log", {})},
        {ErrSlaveThread,                              Message("Could not create slave thread; check system resources", {})},
        {ErrTooManyUserConnections,                   Message("User %-.64s already has more than 'maxUserConnections' active connections", {})},
        ...
        {ErrUnionTablesInDifferentDir,                Message("Incorrect table definition; all MERGE tables must be in the same database", {})},
        ...
        {ErrSpecificAccessDenied,                     Message("Access denied; you need (at least one of) the %-.128s privilege(s) for this operation", {})},
        ...
        {ErrMasterFatalErrorReadingBinlog,            Message("Got fatal error %d from master when reading data from binary log: '%-.320s'", {})},
        ...
        {ErrNotSupportedAuthMode,                     Message("Client does not support authentication protocol requested by server; consider upgrading MySQL client", {})},
        ...
        {ErrTooBigForUncompress,                      Message("Uncompressed data size too large; the maximum size is %d (probably, length of uncompressed data was corrupted)", {})},
        {ErrZlibZMem,                                 Message("ZLIB: Not enough memory", {})},
        {ErrZlibZBuf,                                 Message("ZLIB: Not enough room in the output buffer (probably, length of uncompressed data was corrupted)", {})},
        ...
        {ErrCantAggregate3collations,                 Message("Illegal mix of collations (%s,%s), (%s,%s), (%s,%s) for operation '%s'", {})},
        {ErrCantAggregateNcollations,                 Message("Illegal mix of collations for operation '%s'", {})},
        {ErrVariableIsNotStruct,                      Message("Variable '%-.64s' is not a variable component (can't be used as XXXX.variableName)", {})},
        {ErrUnknownCollation,                         Message("Unknown collation: '%-.64s'", {})},
        {ErrSlaveIgnoredSslParams,                    Message("SSL parameters in CHANGE MASTER are ignored because this MySQL slave was compiled without SSL support; they can be used later if MySQL slave with SSL is started", {})},
        {ErrServerIsInSecureAuthMode,                 Message("Server is running in --secure-auth mode, but '%s'@'%s' has a password in the old format; please change the password to the new format", {})},
        {ErrWarnFieldResolved,                        Message("Field or reference '%-.192s%s%-.192s%s%-.192s' of SELECT #%d was resolved in SELECT #%d", {})},
        {ErrBadSlaveUntilCond,                        Message("Incorrect parameter or combination of parameters for START SLAVE UNTIL", {})},
        {ErrMissingSkipSlave,                         Message("It is recommended to use --skip-slave-start when doing step-by-step replication with START SLAVE UNTIL; otherwise, you will get problems if you get an unexpected slave's mysqld restart", {})},
        ...
        {ErrWarnHostnameWontWork,                     Message("MySQL is started in --skip-name-resolve mode; you must restart it without this switch for this grant to work", {})},
        {ErrUnknownStorageEngine,                     Message("Unknown storage engine '%s'", {})},
        {ErrWarnDeprecatedSyntax,                     Message("'%s' is deprecated and will be removed in a future release. Please use %s instead", {})},
        {ErrNonUpdatableTable,                        Message("The target table %-.100s of the %s is not updatable", {})},
        {ErrFeatureDisabled,                          Message("The '%s' feature is disabled; you need MySQL built with '%s' to have it working", {})},
        {ErrOptionPreventsStatement,                  Message("The MySQL server is running with the %s option so it cannot execute this statement", {})},
        ...
        {ErrTooMuchAutoTimestampCols,                 Message("Incorrect table definition; there can be only one TIMESTAMP column with CURRENTTIMESTAMP in DEFAULT or ON UPDATE clause", {})},
        ...
        {ErrUpdateLogDeprecatedIgnored,               Message("The update log is deprecated and replaced by the binary log; SET SQLLOGUPDATE has been ignored.", {})},
        {ErrUpdateLogDeprecatedTranslated,            Message("The update log is deprecated and replaced by the binary log; SET SQLLOGUPDATE has been translated to SET SQLLOGBIN.", {})},
        ...
        {ErrViewInvalid,                              Message("View '%-.192s.%-.192s' references invalid table(s) or column(s) or function(s) or definer/invoker of view lack rights to use them", {})},
        ...
        {ErrXaerRmfail,                               Message("XAERRMFAIL: The command cannot be executed when global transaction is in the  %.64s state", {})},
        {ErrXaerOutside,                              Message("XAEROUTSIDE: Some work is done outside global transaction", {})},
        {ErrXaerRmerr,                                Message("XAERRMERR: Fatal error occurred in the transaction branch - check your data for consistency", {})},
        {ErrXaRbrollback,                             Message("XARBROLLBACK: Transaction branch was rolled back", {})},
        {ErrNonexistingProcGrant,                     Message("There is no such grant defined for user '%-.48s' on host '%-.64s' on routine '%-.192s'", {})},
        ...
        {ErrSpNotVarArg,                              Message("OUT or INOUT argument %d for routine %s is not a variable or NEW pseudo-variable in BEFORE trigger", {})},
        ...
        {ErrFailedRoutineBreakBinlog,                 Message("A routine failed and has neither NO SQL nor READS SQL DATA in its declaration and binary logging is enabled; if non-transactional tables were updated, the binary log will miss their changes", {})},
        {ErrBinlogUnsafeRoutine,                      Message("This function has none of DETERMINISTIC, NO SQL, or READS SQL DATA in its declaration and binary logging is enabled (you *might* want to use the less safe logBinTrustFunctionCreators variable)", {})},
        {ErrBinlogCreateRoutineNeedSuper,             Message("You do not have the SUPER privilege and binary logging is enabled (you *might* want to use the less safe logBinTrustFunctionCreators variable)", {})},
        {ErrExecStmtWithOpenCursor,                   Message("You can't execute a prepared statement which has an open cursor associated with it. Reset the statement to re-execute it.", {})},
        {ErrStmtHasNoOpenCursor,                      Message("The statement (%d) has no open cursor.", {})},
        {ErrCommitNotAllowedInSfOrTrg,                Message("Explicit or implicit commit is not allowed in stored function or trigger.", {})},
        {ErrNoDefaultForViewField,                    Message("Field of view '%-.192s.%-.192s' underlying table doesn't have a default value", {})},
        ...
        {ErrWrongLockOfSystemTable,                   Message("You can't combine write-locking of system tables with other tables or lock types", {})},
        {ErrConnectToForeignDataSource,               Message("Unable to connect to foreign data source: %.64s", {})},
        {ErrQueryOnForeignDataSource,                 Message("There was a problem processing the query on the foreign data source. Data source : %-.64s", {})},
        {ErrForeignDataSourceDoesntExist,             Message("The foreign data source you are trying to reference does not exist. Data source :  %-.64s", {})},
        {ErrForeignDataStringInvalidCantCreate,       Message("Can't create federated table. The data source connection string '%-.64s' is not in the correct format", {})},
        {ErrForeignDataStringInvalid,                 Message("The data source connection string '%-.64s' is not in the correct format", {})},
        ...
        {ErrStackOverrunNeedMore,                     Message("Thread stack overrun:  %d bytes used of a %d byte stack, and %d bytes needed.  Use 'mysqld --threadStack=#' to specify a bigger stack.", {})},
        ...
        {ErrCantUpdateUsedTableInSfOrTrg,             Message("Can't update table '%-.192s' in stored function/trigger because it is already used by statement which invoked this stored function/trigger.", {})},
        {ErrViewPreventUpdate,                        Message("The definition of table '%-.192s' prevents operation %.192s on table '%-.192s'.", {})},
        {ErrPsNoRecursion,                            Message("The prepared statement contains a stored routine call that refers to that same statement. It's not allowed to execute a prepared statement in such a recursive manner", {})},
        ...
        {ErrViewFrmNoUser,                            Message("View '%-.192s'.'%-.192s' has no definer information (old table format). Current user is used as definer. Please recreate the view!", {})},
        ...
        {ErrTrgNoDefiner,                             Message("No definer attribute for trigger '%-.192s'.'%-.192s'. The trigger will be activated under the authorization of the invoker, which may have insufficient privileges. Please recreate the trigger.", {})},
        {ErrOldFileFormat,                            Message("'%-.192s' has an old format, you should re-create the '%s' object(s)", {})},
        {ErrSpRecursionLimit,                         Message("Recursive limit %d (as set by the maxSpRecursionDepth variable) was exceeded for routine %.192s", {})},
        {ErrSpProcTableCorrupt,                       Message("Failed to load routine %-.192s. The table mysql.proc is missing, corrupt, or contains bad data (internal code %d)", {})},
        ...
        {ErrMaxPreparedStmtCountReached,              Message("Can't create more than maxPreparedStmtCount statements (current value: %d)", {})},
        ...
        {ErrAdminWrongMrgTable,                       Message("Table '%-.64s' is differently defined or of non-MyISAM type or doesn't exist", {})},
        ...
        {ErrAmbiguousFieldTerm,                       Message("First character of the FIELDS TERMINATED string is ambiguous; please use non-optional and non-empty FIELDS ENCLOSED BY", {})},
        {ErrForeignServerExists,                      Message("The foreign server, %s, you are trying to create already exists.", {})},
        {ErrForeignServerDoesntExist,                 Message("The foreign server name you are trying to reference does not exist. Data source :  %-.64s", {})},
        {ErrIllegalHaCreateOption,                    Message("Table storage engine '%-.64s' does not support the create option '%.64s'", {})},
        {ErrPartitionRequiresValues,                  Message("Syntax : %-.64s PARTITIONING requires definition of VALUES %-.64s for each partition", {})},
        ...
        {ErrWrongExprInPartitionFunc,                 Message("Constant, random or timezone-dependent expressions in (sub)partitioning function are not allowed", {})},
        ...
        {ErrInconsistentPartitionInfo,                Message("The partition info in the frm file is not consistent with what can be written into the frm file", {})},
        ...
        {ErrSubpartition,                             Message("It is only possible to mix RANGE/LIST partitioning with HASH/KEY partitioning for subpartitioning", {})},
        ...
        {ErrUniqueKeyNeedAllFieldsInPf,               Message("A %-.192s must include all columns in the table's partitioning function", {})},
        ...
        {ErrForeignKeyOnPartitioned,                  Message("Foreign key clause is not yet supported in conjunction with partitioning", {})},
        ...
        {ErrReorgHashOnlyOnSameNo,                    Message("REORGANIZE PARTITION can only be used to reorganize partitions not to change their numbers", {})},
        {ErrReorgNoParam,                             Message("REORGANIZE PARTITION without parameters can only be used on auto-partitioned tables using HASH PARTITIONs", {})},
        ...
        {ErrConsecutiveReorgPartitions,               Message("When reorganizing a set of partitions they must be in consecutive order", {})},
        {ErrReorgOutsideRange,                        Message("Reorganize of range partitions cannot change total ranges except for last partition where it can extend the range", {})},
        ...
        {ErrSizeOverflow,                             Message("The size number was correct but we don't allow the digit part to be more than 2 billion", {})},
        ...
        {ErrBinlogRowRbrToSbr,                        Message("Slave running with --log-slave-updates must use row-based binary logging to be able to replicate row-based binary log events", {})},
        ...
        {ErrObsoleteColCountDoesntMatchCorrupted,     Message("Column count of mysql.%s is wrong. Expected %d, found %d. The table is probably corrupted", {})},
        ...
        {ErrWarnDeprecatedSyntaxWithVer,              Message("The syntax '%s' is deprecated and will be removed in MySQL %s. Please use %s instead", {})},
        ...
        {ErrForeignDuplicateKeyOldUnused,             Message("Upholding foreign key constraints for table '%.192s', entry '%-.192s', key %d would lead to a duplicate entry", {})},
        {ErrColCountDoesntMatchPleaseUpdate,          Message("Column count of mysql.%s is wrong. Expected %d, found %d. Created with MySQL %d, now running %d. Please use mysqlUpgrade to fix this error.", {})},
        {ErrTempTablePreventsSwitchOutOfRbr,          Message("Cannot switch out of the row-based binary log format when the session has open temporary tables", {})},
        {ErrStoredFunctionPreventsSwitchBinlogFormat, Message("Cannot change the binary logging format inside a stored function or trigger", {})},
        {ErrNdbCantSwitchBinlogFormat,                Message("The NDB cluster engine does not support changing the binlog format on the fly yet", {})},
        ...
        {ErrCantChangeTxCharacteristics,              Message("Transaction characteristics can't be changed while a transaction is in progress", {})},
        {ErrDupEntryAutoincrementCase,                Message("ALTER TABLE causes autoIncrement resequencing, resulting in duplicate entry '%-.192s' for key '%-.192s'", {})},
        ...
        {ErrEventsDB,                                 Message("Cannot proceed because system tables used by Event Scheduler were found damaged at server start", {})},
        ...
        {ErrCantRenameLogTable,                       Message("Cannot rename '%s'. When logging enabled, rename to/from log table must rename two tables: the log table to an archive table and another table back to '%s'", {})},
        ...
        {ErrEventCannotCreateInThePast,               Message("Event execution time is in the past and ON COMPLETION NOT PRESERVE is set. The event was dropped immediately after creation.", {})},
        {ErrEventCannotAlterInThePast,                Message("Event execution time is in the past and ON COMPLETION NOT PRESERVE is set. The event was not changed. Specify a time in the future.", {})},
        ...
        {ErrBinlogUnsafeStatement,                    Message("Unsafe statement written to the binary log using statement format since BINLOGFORMAT = STATEMENT. %s", {})},
        ...
        {ErrNoFormatDescriptionEventBeforeBinlogStatement,         Message("The BINLOG statement of type `%s` was not preceded by a format description BINLOG statement.", {})},
        ...
        {ErrWarnEngineTransactionRollback,                         Message("Storage engine %s does not support rollback for this statement. Transaction rolled back and must be restarted", {})},
        {ErrSlaveHeartbeatFailure,                                 Message("Unexpected master's heartbeat data: %s", {})},
        {ErrSlaveHeartbeatValueOutOfRange,                         Message("The requested value for the heartbeat period is either negative or exceeds the maximum allowed (%s seconds).", {})},
        ...
        {ErrFuncInexistentNameCollision,                           Message("FUNCTION %s does not exist. Check the 'Function Name Parsing and Resolution' section in the Reference Manual", {})},
        ...
        {ErrSlaveIgnoreServerIds,                                  Message("The requested server id %d clashes with the slave startup option --replicate-same-server-id", {})},
        {ErrQueryCacheDisabled,                                    Message("Query cache is disabled; restart the server with queryCacheType=1 to enable it", {})},
        ...
        {ErrRowSinglePartitionField,                               Message("Row expressions in VALUES IN only allowed for multi-field column partitioning", {})},
        {ErrFieldTypeNotAllowedAsPartitionField,                   Message("Field '%-.192s' is of a not allowed type for this type of partitioning", {})},
        {ErrPartitionFieldsTooLong,                                Message("The total length of the partitioning fields is too large", {})},
        {ErrBinlogRowEngineAndStmtEngine,                          Message("Cannot execute statement: impossible to write to binary log since both row-incapable engines and statement-incapable engines are involved.", {})},
        {ErrBinlogRowModeAndStmtEngine,                            Message("Cannot execute statement: impossible to write to binary log since BINLOGFORMAT = ROW and at least one table uses a storage engine limited to statement-based logging.", {})},
        {ErrBinlogUnsafeAndStmtEngine,                             Message("Cannot execute statement: impossible to write to binary log since statement is unsafe, storage engine is limited to statement-based logging, and BINLOGFORMAT = MIXED. %s", {})},
        {ErrBinlogRowInjectionAndStmtEngine,                       Message("Cannot execute statement: impossible to write to binary log since statement is in row format and at least one table uses a storage engine limited to statement-based logging.", {})},
        {ErrBinlogStmtModeAndRowEngine,                            Message("Cannot execute statement: impossible to write to binary log since BINLOGFORMAT = STATEMENT and at least one table uses a storage engine limited to row-based logging.%s", {})},
        {ErrBinlogRowInjectionAndStmtMode,                         Message("Cannot execute statement: impossible to write to binary log since statement is in row format and BINLOGFORMAT = STATEMENT.", {})},
        {ErrBinlogMultipleEnginesAndSelfLoggingEngine,             Message("Cannot execute statement: impossible to write to binary log since more than one engine is involved and at least one engine is self-logging.", {})},
        {ErrBinlogUnsafeLimit,                                     Message("The statement is unsafe because it uses a LIMIT clause. This is unsafe because the set of rows included cannot be predicted.", {})},
        {ErrBinlogUnsafeInsertDelayed,                             Message("The statement is unsafe because it uses INSERT DELAYED. This is unsafe because the times when rows are inserted cannot be predicted.", {})},
        {ErrBinlogUnsafeSystemTable,                               Message("The statement is unsafe because it uses the general log, slow query log, or performanceSchema table(s). This is unsafe because system tables may differ on slaves.", {})},
        {ErrBinlogUnsafeAutoincColumns,                            Message("Statement is unsafe because it invokes a trigger or a stored function that inserts into an AUTOINCREMENT column. Inserted values cannot be logged correctly.", {})},
        {ErrBinlogUnsafeUdf,                                       Message("Statement is unsafe because it uses a UDF which may not return the same value on the slave.", {})},
        {ErrBinlogUnsafeSystemVariable,                            Message("Statement is unsafe because it uses a system variable that may have a different value on the slave.", {})},
        {ErrBinlogUnsafeSystemFunction,                            Message("Statement is unsafe because it uses a system function that may return a different value on the slave.", {})},
        {ErrBinlogUnsafeNontransAfterTrans,                        Message("Statement is unsafe because it accesses a non-transactional table after accessing a transactional table within the same transaction.", {})},
        {ErrMessageAndStatement,                                   Message("%s Statement: %s", {})},
        {ErrSlaveConversionFailed,                                 Message("Column %d of table '%-.192s.%-.192s' cannot be converted from type '%-.32s' to type '%-.32s'", {})},
        {ErrSlaveCantCreateConversion,                             Message("Can't create conversion table for table '%-.192s.%-.192s'", {})},
        {ErrInsideTransactionPreventsSwitchBinlogFormat,           Message("Cannot modify @@session.binlogFormat inside a transaction", {})},
        ...
        {ErrWarnISSkippedTable,                                    Message("Table '%s'.'%s' was skipped since its definition is being modified by concurrent DDL statement", {})},
        {ErrInsideTransactionPreventsSwitchBinlogDirect,           Message("Cannot modify @@session.binlogDirectNonTransactionalUpdates inside a transaction", {})},
        {ErrStoredFunctionPreventsSwitchBinlogDirect,              Message("Cannot change the binlog direct flag inside a stored function or trigger", {})},
        ...
        {ErrBinlogUnsafeMultipleEnginesAndSelfLoggingEngine,       Message("Mixing self-logging and non-self-logging engines in a statement is unsafe.", {})},
        {ErrBinlogUnsafeMixedStatement,                            Message("Statement accesses nontransactional table as well as transactional or temporary table, and writes to any of them.", {})},
        {ErrInsideTransactionPreventsSwitchSQLLogBin,              Message("Cannot modify @@session.sqlLogBin inside a transaction", {})},
        {ErrStoredFunctionPreventsSwitchSQLLogBin,                 Message("Cannot change the sqlLogBin inside a stored function or trigger", {})},
        ...
        {ErrGrantPluginUserExists,                                 Message("GRANT with IDENTIFIED WITH is illegal because the user %-.*s already exists", {})},
        ...
        {ErrSlaveHeartbeatValueOutOfRangeMin,                      Message("The requested value for the heartbeat period is less than 1 millisecond. The value is reset to 0, meaning that heartbeating will effectively be disabled.", {})},
        {ErrSlaveHeartbeatValueOutOfRangeMax,                      Message("The requested value for the heartbeat period exceeds the value of `slaveNetTimeout' seconds. A sensible value for the period should be less than the timeout.", {})},
        ...
        {ErrMultiUpdateKeyConflict,                                Message("Primary key/partition key update is not allowed since the table is updated both as '%-.192s' and '%-.192s'.", {})},
        {ErrTableNeedsRebuild,                                     Message("Table rebuild required. Please do \"ALTER TABLE `%-.32s` FORCE\" or dump/reload to fix it!", {})},
        ...
        {ErrBinlogUnsafeInsertIgnoreSelect,                        Message("INSERT IGNORE... SELECT is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are ignored. This order cannot be predicted and may differ on master and the slave.", {})},
        {ErrBinlogUnsafeInsertSelectUpdate,                        Message("INSERT... SELECT... ON DUPLICATE KEY UPDATE is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are updated. This order cannot be predicted and may differ on master and the slave.", {})},
        {ErrBinlogUnsafeReplaceSelect,                             Message("REPLACE... SELECT is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are replaced. This order cannot be predicted and may differ on master and the slave.", {})},
        {ErrBinlogUnsafeCreateIgnoreSelect,                        Message("CREATE... IGNORE SELECT is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are ignored. This order cannot be predicted and may differ on master and the slave.", {})},
        {ErrBinlogUnsafeCreateReplaceSelect,                       Message("CREATE... REPLACE SELECT is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are replaced. This order cannot be predicted and may differ on master and the slave.", {})},
        {ErrBinlogUnsafeUpdateIgnore,                              Message("UPDATE IGNORE is unsafe because the order in which rows are updated determines which (if any) rows are ignored. This order cannot be predicted and may differ on master and the slave.", {})},
        {ErrPluginNoUninstall,                                     Message("Plugin '%s' is marked as not dynamically uninstallable. You have to stop the server to uninstall it.", {})},
        {ErrPluginNoInstall,                                       Message("Plugin '%s' is marked as not dynamically installable. You have to stop the server to install it.", {})},
        {ErrBinlogUnsafeWriteAutoincSelect,                        Message("Statements writing to a table with an auto-increment column after selecting from another table are unsafe because the order in which rows are retrieved determines what (if any) rows will be written. This order cannot be predicted and may differ on master and the slave.", {})},
        {ErrBinlogUnsafeCreateSelectAutoinc,                       Message("CREATE TABLE... SELECT...  on a table with an auto-increment column is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are inserted. This order cannot be predicted and may differ on master and the slave.", {})},
        {ErrBinlogUnsafeInsertTwoKeys,                             Message("INSERT... ON DUPLICATE KEY UPDATE  on a table with more than one UNIQUE KEY is unsafe", {})},
        ...
        {ErrBinlogUnsafeAutoincNotFirst,                           Message("INSERT into autoincrement field which is not the first part in the composed primary key is unsafe.", {})},
        ...
        {ErrOnlyFdAndRbrEventsAllowedInBinlogStatement,            Message("Only FormatDescriptionLogEvent and row events are allowed in BINLOG statements (but %s was provided)", {})},
        ...
        {ErrBinlogCacheSizeGreaterThanMax,                         Message("Option binlogCacheSize (%d) is greater than maxBinlogCacheSize (%d); setting binlogCacheSize equal to maxBinlogCacheSize.", {})},
        {ErrWarnIndexNotApplicable,                                Message("Cannot use %-.64s access on index '%-.64s' due to type or collation conversion on field '%-.64s'", {})},
        {ErrPartitionExchangeForeignKey,                           Message("Table to exchange with partition has foreign key references: '%-.64s'", {})},
        ...
        {ErrNetworkReadEventChecksumFailure,                       Message("Replication event checksum verification failed while reading from network.", {})},
        {ErrBinlogReadEventChecksumFailure,                        Message("Replication event checksum verification failed while reading from a log file.", {})},
        {ErrBinlogStmtCacheSizeGreaterThanMax,                     Message("Option binlogStmtCacheSize (%d) is greater than maxBinlogStmtCacheSize (%d); setting binlogStmtCacheSize equal to maxBinlogStmtCacheSize.", {})},
        {ErrCantUpdateTableInCreateTableSelect,                    Message("Can't update table '%-.192s' while '%-.192s' is being created.", {})},
        ...
        {ErrWarningNotCompleteRollbackWithCreatedTempTable,        Message("The creation of some temporary tables could not be rolled back.", {})},
        {ErrWarningNotCompleteRollbackWithDroppedTempTable,        Message("Some temporary tables were dropped, but these operations could not be rolled back.", {})},
        {ErrMtsFeatureIsNotSupported,                              Message("%s is not supported in multi-threaded slave mode. %s", {})},
        {ErrMtsUpdatedDBsGreaterMax,                               Message("The number of modified databases exceeds the maximum %d; the database names will not be included in the replication event metadata.", {})},
        {ErrMtsCantParallel,                                       Message("Cannot execute the current event group in the parallel mode. Encountered event %s, relay-log name %s, position %s which prevents execution of this event group in parallel mode. Reason: %s.", {})},
        ...
        {ErrInsecureChangeMaster,                                  Message("Storing MySQL user name or password information in the master.info repository is not secure and is therefore not recommended. Please see the MySQL Manual for more about this issue and possible alternatives.", {})},
        {ErrForeignDuplicateKeyWithChildInfo,                      Message("Foreign key constraint for table '%.192s', record '%-.192s' would lead to a duplicate entry in table '%.192s', key '%.192s'", {})},
        {ErrForeignDuplicateKeyWithoutChildInfo,                   Message("Foreign key constraint for table '%.192s', record '%-.192s' would lead to a duplicate entry in a child table", {})},
        {ErrSQLthreadWithSecureSlave,                              Message("Setting authentication options is not possible when only the Slave SQL Thread is being started.", {})},
        {ErrTableHasNoFt,                                          Message("The table does not have FULLTEXT index to support this query", {})},
        {ErrVariableNotSettableInSfOrTrigger,                      Message("The system variable %.200s cannot be set in stored functions or triggers.", {})},
        {ErrVariableNotSettableInTransaction,                      Message("The system variable %.200s cannot be set when there is an ongoing transaction.", {})},
        {ErrGtidNextIsNotInGtidNextList,                           Message("The system variable @@SESSION.GTIDNEXT has the value %.200s, which is not listed in @@SESSION.GTIDNEXTLIST.", {})},
        {ErrCantChangeGtidNextInTransactionWhenGtidNextListIsNull, Message("When @@SESSION.GTIDNEXTLIST == NULL, the system variable @@SESSION.GTIDNEXT cannot change inside a transaction.", {})},
        {ErrSetStatementCannotInvokeFunction,                      Message("The statement 'SET %.200s' cannot invoke a stored function.", {})},
        {ErrGtidNextCantBeAutomaticIfGtidNextListIsNonNull,        Message("The system variable @@SESSION.GTIDNEXT cannot be 'AUTOMATIC' when @@SESSION.GTIDNEXTLIST is non-NULL.", {})},
        {ErrSkippingLoggedTransaction,                             Message("Skipping transaction %.200s because it has already been executed and logged.", {})},
        ...
        {ErrGnoExhausted,                                          Message("Impossible to generate Global Transaction Identifier: the integer component reached the maximal value. Restart the server with a new serverUuid.", {})},
        {ErrBadSlaveAutoPosition,                                  Message("Parameters MASTERLOGFILE, MASTERLOGPOS, RELAYLOGFILE and RELAYLOGPOS cannot be set when MASTERAUTOPOSITION is active.", {})},
        {ErrAutoPositionRequiresGtidModeOn,                        Message("CHANGE MASTER TO MASTERAUTOPOSITION = 1 can only be executed when @@GLOBAL.GTIDMODE = ON.", {})},
        {ErrCantDoImplicitCommitInTrxWhenGtidNextIsSet,            Message("Cannot execute statements with implicit commit inside a transaction when @@SESSION.GTIDNEXT != AUTOMATIC or @@SESSION.GTIDNEXTLIST != NULL.", {})},
        {ErrGtidMode2Or3RequiresEnforceGtidConsistencyOn,          Message("@@GLOBAL.GTIDMODE = ON or UPGRADESTEP2 requires @@GLOBAL.ENFORCEGTIDCONSISTENCY = 1.", {})},
        {ErrGtidModeRequiresBinlog,                                Message("@@GLOBAL.GTIDMODE = ON or UPGRADESTEP1 or UPGRADESTEP2 requires --log-bin and --log-slave-updates.", {})},
        {ErrCantSetGtidNextToGtidWhenGtidModeIsOff,                Message("@@SESSION.GTIDNEXT cannot be set to UUID:NUMBER when @@GLOBAL.GTIDMODE = OFF.", {})},
        {ErrCantSetGtidNextToAnonymousWhenGtidModeIsOn,            Message("@@SESSION.GTIDNEXT cannot be set to ANONYMOUS when @@GLOBAL.GTIDMODE = ON.", {})},
        {ErrCantSetGtidNextListToNonNullWhenGtidModeIsOff,         Message("@@SESSION.GTIDNEXTLIST cannot be set to a non-NULL value when @@GLOBAL.GTIDMODE = OFF.", {})},
        {ErrFoundGtidEventWhenGtidModeIsOff,                       Message("Found a GtidLogEvent or PreviousGtidsLogEvent when @@GLOBAL.GTIDMODE = OFF.", {})},
        {ErrGtidUnsafeNonTransactionalTable,                       Message("When @@GLOBAL.ENFORCEGTIDCONSISTENCY = 1, updates to non-transactional tables can only be done in either autocommitted statements or single-statement transactions, and never in the same statement as updates to transactional tables.", {})},
        {ErrGtidUnsafeCreateSelect,                                Message("CREATE TABLE ... SELECT is forbidden when @@GLOBAL.ENFORCEGTIDCONSISTENCY = 1.", {})},
        {ErrGtidUnsafeCreateDropTemporaryTableInTransaction,       Message("When @@GLOBAL.ENFORCEGTIDCONSISTENCY = 1, the statements CREATE TEMPORARY TABLE and DROP TEMPORARY TABLE can be executed in a non-transactional context only, and require that AUTOCOMMIT = 1.", {})},
        {ErrGtidModeCanOnlyChangeOneStepAtATime,                   Message("The value of @@GLOBAL.GTIDMODE can only change one step at a time: OFF <-> UPGRADESTEP1 <-> UPGRADESTEP2 <-> ON. Also note that this value must be stepped up or down simultaneously on all servers; see the Manual for instructions.", {})},
        {ErrMasterHasPurgedRequiredGtids,                          Message("The slave is connecting using CHANGE MASTER TO MASTERAUTOPOSITION = 1, but the master has purged binary logs containing GTIDs that the slave requires.", {})},
        {ErrCantSetGtidNextWhenOwningGtid,                         Message("@@SESSION.GTIDNEXT cannot be changed by a client that owns a GTID. The client owns %s. Ownership is released on COMMIT or ROLLBACK.", {})},
        ...
        {ErrSlaveConfiguration,                                    Message("Slave is not configured or failed to initialize properly. You must at least set --server-id to enable either a master or a slave. Additional error messages can be found in the MySQL error log.", {})},
        ...
        {ErrInnodbOnlineLogTooBig,                                 Message("Creating index '%-.192s' required more than 'innodbOnlineAlterLogMaxSize' bytes of modification log. Please try again.", {})},
        {ErrUnknownAlterAlgorithm,                                 Message("Unknown ALGORITHM '%s'", {})},
        {ErrUnknownAlterLock,                                      Message("Unknown LOCK type '%s'", {})},
        {ErrMtsChangeMasterCantRunWithGaps,                        Message("CHANGE MASTER cannot be executed when the slave was stopped with an error or killed in MTS mode. Consider using RESET SLAVE or START SLAVE UNTIL.", {})},
        {ErrMtsRecoveryFailure,                                    Message("Cannot recover after SLAVE errored out in parallel execution mode. Additional error messages can be found in the MySQL error log.", {})},
        {ErrMtsResetWorkers,                                       Message("Cannot clean up worker info tables. Additional error messages can be found in the MySQL error log.", {})},
        {ErrColCountDoesntMatchCorruptedV2,                        Message("Column count of %s.%s is wrong. Expected %d, found %d. The table is probably corrupted", {})},
        {ErrSlaveSilentRetryTransaction,                           Message("Slave must silently retry current transaction", {})},
        {ErrDiscardFkChecksRunning,                                Message("There is a foreign key check running on table '%-.192s'. Cannot discard the table.", {})},
        ...
        {ErrTablespaceExists,                                      Message("Tablespace for table '%-.192s' exists. Please DISCARD the tablespace before IMPORT.", {})},
        ...
        {ErrFkNoIndexChild,                                        Message("Failed to add the foreign key constaint. Missing index for constraint '%s' in the foreign table '%s'", {})},
        {ErrFkNoIndexParent,                                       Message("Failed to add the foreign key constaint. Missing index for constraint '%s' in the referenced table '%s'", {})},
        ...
        {ErrFkIncorrectOption,                                     Message("Failed to add the foreign key constraint on table '%s'. Incorrect options in FOREIGN KEY constraint '%s'", {})},
        {ErrFkDupName,                                             Message("Duplicate foreign key constraint name '%s'", {})},
        {ErrPasswordFormat,                                        Message("The password hash doesn't have the expected format. Check if the correct password algorithm is being used with the PASSWORD() function.", {})},
        {ErrFkColumnCannotDrop,                                    Message("Cannot drop column '%-.192s': needed in a foreign key constraint '%-.192s'", {})},
        {ErrFkColumnCannotDropChild,                               Message("Cannot drop column '%-.192s': needed in a foreign key constraint '%-.192s' of table '%-.192s'", {})},
        {ErrFkColumnNotNull,                                       Message("Column '%-.192s' cannot be NOT NULL: needed in a foreign key constraint '%-.192s' SET NULL", {})},
        {ErrDupIndex,                                              Message("Duplicate index '%-.64s' defined on the table '%-.64s.%-.64s'. This is deprecated and will be disallowed in a future release.", {})},
        {ErrFkColumnCannotChange,                                  Message("Cannot change column '%-.192s': used in a foreign key constraint '%-.192s'", {})},
        {ErrFkColumnCannotChangeChild,                             Message("Cannot change column '%-.192s': used in a foreign key constraint '%-.192s' of table '%-.192s'", {})},
        {ErrFkCannotDeleteParent,                                  Message("Cannot delete rows from table which is parent in a foreign key constraint '%-.192s' of table '%-.192s'", {})},
        ...
        {ErrCantSetGtidPurgedWhenGtidModeIsOff,                    Message("@@GLOBAL.GTIDPURGED can only be set when @@GLOBAL.GTIDMODE = ON.", {})},
        {ErrCantSetGtidPurgedWhenGtidExecutedIsNotEmpty,           Message("@@GLOBAL.GTIDPURGED can only be set when @@GLOBAL.GTIDEXECUTED is empty.", {})},
        {ErrCantSetGtidPurgedWhenOwnedGtidsIsNotEmpty,             Message("@@GLOBAL.GTIDPURGED can only be set when there are no ongoing transactions (not even in other clients).", {})},
        ...
        {ErrBinlogStmtModeAndNoReplTables,                         Message("Cannot execute statement: impossible to write to binary log since BINLOGFORMAT = STATEMENT, and both replicated and non replicated tables are written to.", {})},
        ...
        {ErrAlterOperationNotSupportedReasonPartition,             Message("Partition specific operations do not yet support LOCK/ALGORITHM", {})},
        ...
        {ErrAlterOperationNotSupportedReasonIgnore,                Message("Creating unique indexes with IGNORE requires COPY algorithm to remove duplicate rows", {})},
        {ErrAlterOperationNotSupportedReasonNopk,                  Message("Dropping a primary key is not allowed without also adding a new primary key", {})},
        {ErrAlterOperationNotSupportedReasonAutoinc,               Message("Adding an auto-increment column requires a lock", {})},
        {ErrAlterOperationNotSupportedReasonHiddenFts,             Message("Cannot replace hidden FTSDOCID with a user-visible one", {})},
        ...
        {ErrSQLSlaveSkipCounterNotSettableInGtidMode,              Message("sqlSlaveSkipCounter can not be set when the server is running with @@GLOBAL.GTIDMODE = ON. Instead, for each transaction that you want to skip, generate an empty transaction with the same GTID as the transaction", {})},
        {ErrDupUnknownInIndex,                                     Message("Duplicate entry for key '%-.192s'", {})},
        {ErrIdentCausesTooLongPath,                                Message("Long database name and identifier for object resulted in path length exceeding %d characters. Path: '%s'.", {})},
        {ErrAlterOperationNotSupportedReasonNotNull,               Message("cannot silently convert NULL values, as required in this SQLMODE", {})},
        {ErrMustChangePasswordLogin,                               Message("Your password has expired. To log in you must change it using a client that supports expired passwords.", {})},
        {ErrRowInWrongPartition,                                   Message("Found a row in wrong partition %s", {})},
        {ErrGeneratedColumnFunctionIsNotAllowed,                   Message("Expression of generated column '%s' contains a disallowed function.", {})},
        {ErrUnsupportedAlterInplaceOnVirtualColumn,                Message("INPLACE ADD or DROP of virtual columns cannot be combined with other ALTER TABLE actions.", {})},
        {ErrWrongFKOptionForGeneratedColumn,                       Message("Cannot define foreign key with %s clause on a generated column.", {})},
        ...
        {ErrGeneratedColumnNonPrior,                               Message("Generated column can refer only to generated columns defined prior to it.", {})},
        ...
        {ErrInvalidTypeForJSON,                                    Message("Invalid data type for JSON data in argument %d to function %s; a JSON string or JSON type is required.", {})},
        {ErrInvalidJSONPathWildcard,                               Message("In this situation, path expressions may not contain the * and ** tokens.", {})},
        ...
        {ErrWindowNoInherentFrame,                                 Message("Window '%s' has a frame definition, so cannot be referenced by another window.", {})},
        ...
        {ErrWindowRangeFrameOrderType,                             Message("Window '%s' with RANGE N PRECEDING/FOLLOWING frame requires exactly one ORDER BY expression, of numeric or temporal type", {})},
        {ErrWindowRangeFrameTemporalType,                          Message("Window '%s' with RANGE frame has ORDER BY expression of datetime type. Only INTERVAL bound value allowed.", {})},
        {ErrWindowRangeFrameNumericType,                           Message("Window '%s' with RANGE frame has ORDER BY expression of numeric type, INTERVAL bound value not allowed.", {})},
        ...
        {ErrWindowIllegalOrderBy,                                  Message("Window '%s': ORDER BY or PARTITION BY uses legacy position indication which is not supported, use expression.", {})},
        {ErrWindowInvalidWindowFuncUse,                            Message("You cannot use the window function '%s' in this context.'", {})},
        {ErrWindowInvalidWindowFuncAliasUse,                       Message("You cannot use the alias '%s' of an expression containing a window function in this context.'", {})},
        {ErrWindowNestedWindowFuncUseInWindowSpec,                 Message("You cannot nest a window function in the specification of window '%s'.", {})},
        {ErrWindowRowsIntervalUse,                                 Message("Window '%s': INTERVAL can only be used with RANGE frames.", {})},
        {ErrWindowNoGroupOrderUnused,                              Message("ASC or DESC with GROUP BY isn't allowed with window functions; put ASC or DESC in ORDER BY", {})},
        {ErrWindowExplainJson,                                     Message("To get information about window functions use EXPLAIN FORMAT=JSON", {})},
        {ErrWindowFunctionIgnoresFrame,                            Message("Window function '%s' ignores the frame clause of window '%s' and aggregates over the whole partition", {})},
        ...
        {ErrLockAcquireFailAndNoWaitSet,                           Message("Statement aborted because lock(s) could not be acquired immediately and NOWAIT is set.", {})},
        ...
        {ErrFunctionalIndexOnJsonOrGeometryFunction,               Message("Cannot create a functional index on a function that returns a JSON or GEOMETRY value", {})},
        {ErrFunctionalIndexRefAutoIncrement,                       Message("Functional index '%s' cannot refer to an auto-increment column", {})},
        {ErrCannotDropColumnFunctionalIndex,                       Message("Cannot drop column '%s' because it is used by a functional index. In order to drop the column, you must remove the functional index", {})},
        {ErrFunctionalIndexPrimaryKey,                             Message("The primary key cannot be a functional index", {})},
        {ErrFunctionalIndexOnLob,                                  Message("Cannot create a functional index on an expression that returns a BLOB or TEXT. Please consider using CAST", {})},
        {ErrFunctionalIndexFunctionIsNotAllowed,                   Message("Expression of functional index '%s' contains a disallowed function", {})},
        ...
        {ErrFunctionalIndexOnField,                                Message("Functional index on a column is not supported. Consider using a regular index instead", {})},
        {ErrFKIncompatibleColumns,                                 Message("Referencing column '%s' in foreign key constraint '%s' are incompatible", {})},
        {ErrFunctionalIndexRowValueIsNotAllowed,                   Message("Expression of functional index '%s' cannot refer to a row value", {})},
        {ErrDependentByFunctionalIndex,                            Message("Column '%s' has a functional index dependency and cannot be dropped or renamed", {})},
        ...
        {ErrFunctionalIndexNotApplicable,                          Message("Cannot use functional index '%s' due to type or collation conversion", {})},
        ...
        {ErrSystemVersioningWrongPartitions,      Message("Wrong Partitions: must have at least one HISTORY and exactly one last CURRENT", {})},
        ...
        {ErrWrongInsertIntoSequence,              Message("Wrong INSERT into a SEQUENCE. One can only do single table INSERT into a sequence object (like with mysqldump).  If you want to change the SEQUENCE, use ALTER SEQUENCE instead.", {})},
